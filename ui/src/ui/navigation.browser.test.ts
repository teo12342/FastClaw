...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/fastclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/fastclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/fastclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__FASTCLAW_CONTROL_UI_BASE_PATH__ = "/fastclaw";
    const app = mountApp("/fastclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/fastclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/fastclaw/sessions");
  });
