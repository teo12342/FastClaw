...
export async function handleWhatsAppStart(host: FastClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: FastClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: FastClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: FastClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: FastClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: FastClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: FastClawApp): string | null {
...
function buildGatewayHttpHeaders(host: FastClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: FastClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: FastClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: FastClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: FastClawApp) {
  const state = host.nostrProfileFormState;
