...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/fastclaw`)
...
If the page fails to load, start the Gateway first: `fastclaw gateway`.
...
you revoke it with `fastclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.fastclaw/fastclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop fastclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. FastClaw
...
```bash
fastclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
FastClaw **blocks** Control UI connections without device identity.
...
```bash
FASTCLAW_CONTROL_UI_BASE_PATH=/fastclaw/ pnpm ui:build
```
