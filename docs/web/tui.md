...
```bash
fastclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `fastclaw tui --deliver`
...
- Check the Gateway logs: `fastclaw logs --follow`.
- Confirm the agent can run: `fastclaw status` and `fastclaw models status`.
...
- No agents in picker: check `fastclaw agents list` and your routing config.
