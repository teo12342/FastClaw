---
summary: "Use MiniMax M2.1 in FastClaw"
read_when:
  - You want MiniMax models in FastClaw
...
```bash
fastclaw plugins enable minimax-portal-auth  # skip if already loaded.
fastclaw gateway restart  # restart if gateway is already running
fastclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/fastclaw/fastclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `fastclaw.json`:
...
- Use `fastclaw models list` and `fastclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `fastclaw configure` and selecting **MiniMax M2.1**, or
