---
summary: "Use OpenAI via API keys or Codex subscription in FastClaw"
read_when:
  - You want to use OpenAI models in FastClaw
...
```bash
fastclaw onboard --auth-choice openai-api-key
# or non-interactive
fastclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
fastclaw onboard --auth-choice openai-codex

# Or run OAuth directly
fastclaw models auth login --provider openai-codex
```
...
FastClaw uses `pi-ai` for model streaming. For `openai-codex/*` models you can set
...
`baseUrl` on `api.openai.com`), FastClaw now auto-enables OpenAI server-side
