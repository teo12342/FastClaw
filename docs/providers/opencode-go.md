...
```bash
fastclaw onboard --auth-choice opencode-go
# or non-interactive
fastclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
...
FastClaw handles per-model routing automatically when the model ref uses `opencode-go/...`.
