---
summary: "Use Venice AI privacy-focused models in FastClaw"
read_when:
  - You want privacy-focused inference in FastClaw
...
```bash
fastclaw onboard --auth-choice venice-api-key
```
...
```bash
fastclaw agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, FastClaw shows all available Venice models. Pick based on your needs:
...
```bash
fastclaw models set venice/claude-opus-45
fastclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
fastclaw models list | grep venice
```

## Configure via `fastclaw configure`

1. Run `fastclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
FastClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
fastclaw agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
fastclaw agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
fastclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
fastclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
fastclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `fastclaw models list` to see currently available models. Some models may be temporarily offline.
