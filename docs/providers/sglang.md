---
summary: "Run FastClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run FastClaw against a local SGLang server
...
FastClaw can connect to SGLang using the `openai-completions` API.

FastClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, FastClaw will query:
