...
read_when:
  - You want to use Groq with FastClaw
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. FastClaw connects
...
media-understanding provider, FastClaw uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.fastclaw/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `fastclaw models list | grep groq`
