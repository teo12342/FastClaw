...
read_when:
  ...
  - 你想在 FastClaw 中通过 Kilo Gateway 运行模型
summary: 使用 Kilo Gateway 的统一 API 在 FastClaw 中访问多种模型
x-i18n:
...
```bash
fastclaw onboard --kilocode-api-key <key>
```
...
FastClaw 会在启动时从 Kilo Gateway 动态发现可用模型。使用
