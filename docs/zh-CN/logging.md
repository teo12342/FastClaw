...
你可以在 `~/.fastclaw/fastclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.fastclaw/fastclaw.json` 的 `logging` 下。
...
- FastClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `fastclaw plugins enable diagnostics-otel` 启用插件。
...
- `fastclaw.tokens`（计数器，属性：`fastclaw.token`、`fastclaw.channel`、`fastclaw.provider`、`fastclaw.model`）
- `fastclaw.cost.usd`（计数器，属性：`fastclaw.channel`、`fastclaw.provider`、`fastclaw.model`）
- `fastclaw.run.duration_ms`（直方图，属性：`fastclaw.channel`、`fastclaw.provider`、`fastclaw.model`）
- `fastclaw.context.tokens`（直方图，属性：`fastclaw.context`、`fastclaw.channel`、`fastclaw.provider`、`fastclaw.model`）

消息流：

- `fastclaw.webhook.received`（计数器，属性：`fastclaw.channel`、`fastclaw.webhook`）
- `fastclaw.webhook.error`（计数器，属性：`fastclaw.channel`、`fastclaw.webhook`）
- `fastclaw.webhook.duration_ms`（直方图，属性：`fastclaw.channel`、`fastclaw.webhook`）
- `fastclaw.message.queued`（计数器，属性：`fastclaw.channel`、`fastclaw.source`）
- `fastclaw.message.processed`（计数器，属性：`fastclaw.channel`、`fastclaw.outcome`）
- `fastclaw.message.duration_ms`（直方图，属性：`fastclaw.channel`、`fastclaw.outcome`）

队列 + 会话：

- `fastclaw.queue.lane.enqueue`（计数器，属性：`fastclaw.lane`）
- `fastclaw.queue.lane.dequeue`（计数器，属性：`fastclaw.lane`）
- `fastclaw.queue.depth`（直方图，属性：`fastclaw.lane` 或 `fastclaw.channel=heartbeat`）
- `fastclaw.queue.wait_ms`（直方图，属性：`fastclaw.lane`）
- `fastclaw.session.state`（计数器，属性：`fastclaw.state`、`fastclaw.reason`）
- `fastclaw.session.stuck`（计数器，属性：`fastclaw.state`）
- `fastclaw.session.stuck_age_ms`（直方图，属性：`fastclaw.state`）
- `fastclaw.run.attempt`（计数器，属性：`fastclaw.attempt`）
...
- `fastclaw.webhook.error`
  - `fastclaw.channel`、`fastclaw.webhook`、`fastclaw.chatId`、`fastclaw.error`
- `fastclaw.message.processed`
  - `fastclaw.channel`、`fastclaw.outcome`、`fastclaw.chatId`、`fastclaw.messageId`、`fastclaw.sessionKey`、`fastclaw.sessionId`、`fastclaw.reason`
- `fastclaw.session.stuck`
  - `fastclaw.state`、`fastclaw.ageMs`、`fastclaw.queueDepth`、`fastclaw.sessionKey`、`fastclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `fastclaw doctor`。
