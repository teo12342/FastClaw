...
- 默认：`http://<host>:18789/`
- 可选前缀：设置 `gateway.controlUi.basePath`（例如 `/fastclaw`）
...
如果页面加载失败，请先启动 Gateway 网关：`fastclaw gateway`。
...
一旦批准，设备会被记住，除非你使用 `fastclaw devices revoke --device <id> --role <role>` 撤销它，否则不需要重新批准。参见 [Devices CLI](/cli/devices) 了解 token 轮换和撤销。
...
- 配置：查看/编辑 `~/.fastclaw/fastclaw.json`（`config.get`、`config.set`）
...
如果你通过普通 HTTP 打开仪表板（`http://<lan-ip>` 或 `http://<tailscale-ip>`），浏览器在**非安全上下文**中运行并阻止 WebCrypto。默认情况下，FastClaw **阻止**没有设备身份的控制 UI 连接。
