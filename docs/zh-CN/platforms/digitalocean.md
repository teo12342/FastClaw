---
read_when:
  - 在 DigitalOcean 上设置 FastClaw
  - 寻找便宜的 VPS 托管来运行 FastClaw
summary: 在 DigitalOcean 上运行 FastClaw（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 FastClaw

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 FastClaw Gateway 网关。
...
- `~/.fastclaw/` — 配置、凭证、会话数据
- `~/.fastclaw/workspace/` — 工作区（SOUL.md、记忆等）
