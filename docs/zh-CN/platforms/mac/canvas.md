...
- `~/Library/Application Support/FastClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `fastclaw-canvas://<session>/<path>`

示例：

- `fastclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `fastclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `fastclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
fastclaw nodes canvas present --node <id>
fastclaw nodes canvas navigate --node <id> --url "/"
fastclaw nodes canvas eval --node <id> --js "document.title"
fastclaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__fastclaw__/a2ui/
```
...
fastclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
fastclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "fastclaw://agent?message=Review%20this%20design";
```
