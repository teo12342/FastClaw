...
read_when:
  ...
  - You are defining plugin config schemas or package.json fastclaw metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`fastclaw.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `fastclaw` field that tells the plugin system what
...
```json
{
  "name": "@myorg/fastclaw-my-channel",
  "version": "1.0.0",
  "type": "module",
  "fastclaw": {
    "extensions": ["./index.ts"],
...
```json
{
  "name": "@myorg/fastclaw-my-provider",
  "version": "1.0.0",
  "type": "module",
  "fastclaw": {
    "extensions": ["./index.ts"],
...
When enabled, FastClaw loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `fastclaw.plugin.json` in the package root.
FastClaw uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to FastClaw",
  "configSchema": {
    "type": "object",
...
FastClaw loads when it only needs setup surfaces (onboarding, config repair,
...
**When FastClaw uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `fastclaw/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that FastClaw validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "fastclaw/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `fastclaw onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "fastclaw/plugin-sdk/channel-setup";
...
helpers from `fastclaw/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`fastclaw/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `fastclaw/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "fastclaw/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/fastclaw-my-channel",
...
```bash
fastclaw plugins install @myorg/fastclaw-my-plugin
```

FastClaw tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
fastclaw plugins install clawhub:@myorg/fastclaw-my-plugin   # ClawHub only
fastclaw plugins install npm:@myorg/fastclaw-my-plugin       # npm only
```
...
```bash
fastclaw plugins search <query>
fastclaw plugins install <package-name>
```

<Info>
  For npm-sourced installs, `fastclaw plugins install` runs
