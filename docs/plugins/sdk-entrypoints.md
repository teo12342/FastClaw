...
**Import:** `fastclaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `FastClawPluginConfigSchema \| () => FastClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: FastClawPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `fastclaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `fastclaw/plugin-sdk/core`
...
| `registerFull` | `(api: FastClawPluginApi) => void`                               | No       | —                   |
...
FastClaw loads this instead of the full entry when a channel is disabled,
...
FastClaw classifies loaded plugins by their registration behavior:
...
Use `fastclaw plugins inspect <id>` to see a plugin's shape.
