...
summary: "Create your first FastClaw plugin in minutes"
read_when:
  - You want to create a new FastClaw plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to FastClaw
---

# Building Plugins

Plugins extend FastClaw with new capabilities: channels, model providers, speech,
image generation, web search, agent tools, or any combination.

You do not need to add your plugin to the FastClaw repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`fastclaw plugins install <package-name>`. FastClaw tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect FastClaw to a messaging platform (Discord, IRC, etc.)
  </Card>
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json fastclaw.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to FastClaw",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Write the entry point">
    ...
    ```typescript
    // index.ts
    import { definePluginEntry } from "fastclaw/plugin-sdk/plugin-entry";
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** publish to [ClawHub](/tools/clawhub) or npm, then install:
    ...
    FastClaw checks ClawHub first, then falls back to npm.
...
Always import from focused `fastclaw/plugin-sdk/<subpath>` paths:

```typescript
import { definePluginEntry } from "fastclaw/plugin-sdk/plugin-entry";
import { createPluginRuntimeStore } from "fastclaw/plugin-sdk/runtime-store";

// Wrong: monolithic root (deprecated, will be removed)
import { ... } from "fastclaw/plugin-sdk";
```
...
<Check>**package.json** has correct `fastclaw` metadata</Check>
<Check>**fastclaw.plugin.json** manifest is present and valid</Check>
