...
read_when:
  - You want to place an outbound voice call from FastClaw
...
Voice calls for FastClaw via a plugin. Supports outbound notifications and
...
- Use `fastclaw voicecall ...` or the `voice_call` tool
...
```bash
fastclaw plugins install ./extensions/voice-call
