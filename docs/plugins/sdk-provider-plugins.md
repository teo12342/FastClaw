...
summary: "Step-by-step guide to building a model provider plugin for FastClaw"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to FastClaw
...
(LLM) to FastClaw. By the end you will have a provider with a model catalog,
API key auth, and dynamic model resolution.

<Info>
  If you have not built any FastClaw plugin before, read
...
<Steps>
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so FastClaw can detect
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "fastclaw/plugin-sdk/provider-auth";
    ...
    `fastclaw onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "fastclaw/plugin-sdk/provider-entry";
    ...
    `fastclaw/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      FastClaw calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    FastClaw classifies this as a **hybrid-capability** plugin. This is the
...
```
extensions/acme-ai/
├── package.json              # fastclaw.providers metadata
├── fastclaw.plugin.json      # Manifest with providerAuthEnvVars
