...
read_when:
  - Building or debugging native FastClaw plugins
...
This page covers the internal architecture of the FastClaw plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside FastClaw. Every
native FastClaw plugin registers against one or more capability types:
...
FastClaw classifies every loaded plugin into a shape based on its actual
...
Use `fastclaw plugins inspect <id>` to see a plugin's shape and capability
...
When you run `fastclaw doctor` or `fastclaw plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `fastclaw status --all` and `fastclaw plugins doctor`.

## Architecture overview

FastClaw's plugin system has four layers:

1. **Manifest + discovery**
   FastClaw finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `fastclaw.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native FastClaw plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of FastClaw reads the registry to expose tools, channels, provider
...
That split lets FastClaw validate config, explain missing/disabled plugins, and
...
normal chat actions. FastClaw keeps one shared `message` tool in core, and
...
FastClaw treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's FastClaw-facing
  surfaces
...
So if FastClaw adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If FastClaw has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { FastClawPluginDefinition } from "fastclaw/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "fastclaw/plugin-sdk";

const plugin: FastClawPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
FastClaw already treats image/audio/video understanding as one shared
...
If FastClaw adds a new domain later, such as video generation, use the same
...
`FastClawPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   FastClaw can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that FastClaw knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `FastClawPluginApi` or
  `api.runtime`
...
Native FastClaw plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the FastClaw process

Compatible bundles are safer by default because FastClaw currently treats them
...
name: `@fastclaw/<id>` by default, or an approved typed suffix such as
...
FastClaw exports capabilities, not implementation convenience.
...
At startup, FastClaw does roughly this:
...
The manifest is the control-plane source of truth. FastClaw uses it to:
...
FastClaw keeps short in-process caches for:
...
- Set `FASTCLAW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `FASTCLAW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS` and
  `FASTCLAW_PLUGIN_MANIFEST_CACHE_MS`.
...
FastClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, FastClaw calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | FastClaw tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on FastClaw's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before FastClaw's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as FastClaw adds those
...
- If FastClaw adds a new capability such as video generation later, define the
...
- FastClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "FastClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `fastclaw/plugin-sdk` import when
authoring plugins:

- `fastclaw/plugin-sdk/plugin-entry` for plugin registration primitives.
- `fastclaw/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `fastclaw/plugin-sdk/channel-setup`,
  `fastclaw/plugin-sdk/channel-pairing`,
  `fastclaw/plugin-sdk/channel-contract`,
  `fastclaw/plugin-sdk/channel-feedback`,
  `fastclaw/plugin-sdk/channel-inbound`,
  `fastclaw/plugin-sdk/channel-lifecycle`,
  `fastclaw/plugin-sdk/channel-reply-pipeline`,
  `fastclaw/plugin-sdk/command-auth`,
  `fastclaw/plugin-sdk/secret-input`, and
  `fastclaw/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `fastclaw/plugin-sdk/channel-config-helpers`,
  `fastclaw/plugin-sdk/allow-from`,
  `fastclaw/plugin-sdk/channel-config-schema`,
  `fastclaw/plugin-sdk/channel-policy`,
  `fastclaw/plugin-sdk/config-runtime`,
  `fastclaw/plugin-sdk/infra-runtime`,
  `fastclaw/plugin-sdk/agent-runtime`,
  `fastclaw/plugin-sdk/lazy-runtime`,
  `fastclaw/plugin-sdk/reply-history`,
  `fastclaw/plugin-sdk/routing`,
  `fastclaw/plugin-sdk/status-helpers`,
  `fastclaw/plugin-sdk/runtime-store`, and
  `fastclaw/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `fastclaw/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `fastclaw/plugin-sdk/*` subpaths. FastClaw core/test code may use the repo
...
- Avoid the root `fastclaw/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `fastclaw/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `fastclaw/plugin-sdk/channel-actions`.
...
`fastclaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape FastClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to FastClaw's
...
- if both `catalog` and `discovery` are registered, FastClaw uses `catalog`
...
- Read-only command paths such as `fastclaw status`, `fastclaw status --all`,
  `fastclaw channels status`, `fastclaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `fastclaw.extensions`:
...
Security guardrail: every `fastclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `fastclaw plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `fastclaw.setupEntry` can point at a lightweight setup-only module.
When FastClaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `fastclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let FastClaw load the
...
Channel plugins can advertise setup/discovery metadata via `fastclaw.channel` and
install hints via `fastclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@fastclaw/nextcloud-talk",
  "fastclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@fastclaw/nextcloud-talk",
...
FastClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.fastclaw/mpm/plugins.json`
- `~/.fastclaw/mpm/catalog.json`
- `~/.fastclaw/plugins/catalog.json`

Or point `FASTCLAW_PLUGIN_CATALOG_PATHS` (or `FASTCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "fastclaw": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "fastclaw/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `FastClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how FastClaw stays opinionated without becoming hardcoded to one
