...
read_when:
  - You see the FASTCLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the FASTCLAW_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external FastClaw plugin
---

# Plugin SDK Migration

FastClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`fastclaw/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`fastclaw/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`fastclaw/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "fastclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "fastclaw/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "fastclaw/extension-api";
...
```bash
FASTCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 fastclaw gateway run
FASTCLAW_SUPPRESS_EXTENSION_API_WARNING=1 fastclaw gateway run
```
