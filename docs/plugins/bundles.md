---
summary: "Install and use Codex, Claude, and Cursor bundles as FastClaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how FastClaw maps bundle content into native features
...
FastClaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
FastClaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native FastClaw plugins. Native plugins run
...
of requiring authors to rewrite them as native FastClaw plugins, FastClaw
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    fastclaw plugins install ./my-bundle

    # Archive
    fastclaw plugins install ./my-bundle.tgz

    # Claude marketplace
    fastclaw plugins marketplace list <marketplace-name>
    fastclaw plugins install <plugin-name>@<marketplace-name>
    ```
...
## What FastClaw maps from bundles

Not every bundle feature runs in FastClaw today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal FastClaw skills                                                    | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | FastClaw-style `HOOK.md` + `handler.ts` layouts                                                      | Codex          |
...
These are recognized and shown in diagnostics, but FastClaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit FastClaw best when they use skill roots and FastClaw-style
...
FastClaw checks for native plugin format first:

1. `fastclaw.plugin.json` or valid `package.json` with `fastclaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, FastClaw uses the native path. This prevents
...
- FastClaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `fastclaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. FastClaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    FastClaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
