...
read_when:
  ...
  - You want a reference for all registration methods on FastClawPluginApi
...
The `register(api)` callback receives an `FastClawPluginApi` object with these
methods:
...
| `api.config`             | `FastClawConfig`          | Current config snapshot                                   |
...
<Warning>
  Never import your own plugin through `fastclaw/plugin-sdk/<your-plugin>`
