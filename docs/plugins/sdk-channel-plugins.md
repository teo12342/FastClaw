...
summary: "Step-by-step guide to building a messaging channel plugin for FastClaw"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect FastClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects FastClaw to a
...
Channel plugins do not need their own send/edit/react tools. FastClaw keeps one
...
<Steps>
    ...
    ```json package.json
    {
      ...
      "fastclaw": {
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect FastClaw to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "fastclaw/plugin-sdk/core";
  ...
  <Step title="Add a setup entry">
    ...
    FastClaw loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    FastClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to FastClaw.
...
├── package.json              # fastclaw.channel metadata
├── fastclaw.plugin.json      # Manifest with config schema
