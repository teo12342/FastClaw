...
FastClaw can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
...
Edge TTS does **not** require an API key. If no API keys are found, FastClaw defaults
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, FastClaw retries with MP3.
