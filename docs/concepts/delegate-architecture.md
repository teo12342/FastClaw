---
summary: "Delegate architecture: running FastClaw as a named agent on behalf of an organization"
...
Goal: run FastClaw as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an FastClaw agent that:
...
FastClaw's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of FastClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.fastclaw/workspace-delegate",
  tools: {
...
- Cron run history: `~/.fastclaw/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.fastclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through FastClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
fastclaw agents add delegate
```

This creates:

- Workspace: `~/.fastclaw/workspace-delegate`
- State: `~/.fastclaw/agents/delegate/agent`
- Sessions: `~/.fastclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.fastclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.fastclaw/workspace-delegate",
        tools: {
...
~/.fastclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.fastclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.fastclaw/workspace-org",
        agentDir: "~/.fastclaw/agents/org-assistant/agent",
