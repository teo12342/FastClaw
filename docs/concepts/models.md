...
FastClaw selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models FastClaw can use (plus aliases).
...
`fastclaw models` (no subcommand) is a shortcut for `models status`.
...
`fastclaw models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.fastclaw/agents/<agentId>/models.json`). This file
