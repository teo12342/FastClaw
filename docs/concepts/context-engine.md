...
read_when:
  - You want to understand how FastClaw assembles model context
...
A **context engine** controls how FastClaw builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

FastClaw ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.fastclaw/fastclaw.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other FastClaw plugin. Install
first, then select the engine in the slot:

```bash
# Install from npm
fastclaw plugins install @martian-engineering/lossless-claw

# Or install from a local path (for development)
fastclaw plugins install -l ./my-context-engine
```
...
Every time FastClaw runs a model prompt, the context engine participates at
...
FastClaw currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. FastClaw
...
The built-in `legacy` engine preserves FastClaw's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. FastClaw uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. FastClaw disables Pi's built-in
...
`ownsCompaction: false` does **not** mean FastClaw automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `fastclaw/plugin-sdk/core` to use
  FastClaw's built-in compaction behavior.
...
FastClaw resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to FastClaw's built-in summarization. Plugin engines can implement
...
- Use `fastclaw doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, FastClaw
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `fastclaw plugins install -l ./my-engine` to link a
