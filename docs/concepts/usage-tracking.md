...
- `/usage cost` in chats: local cost summary aggregated from FastClaw session logs.
- CLI: `fastclaw status --usage` prints a full per-provider breakdown.
- CLI: `fastclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
