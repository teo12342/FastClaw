---
title: "Memory"
summary: "How FastClaw memory works (workspace files + automatic memory flush)"
read_when:
...
FastClaw memory is **plain Markdown in the agent workspace**. The files are the
...
`~/.fastclaw/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

FastClaw exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, FastClaw triggers a **silent,
...
FastClaw can build a small vector index over `MEMORY.md` and `memory/*.md` so
