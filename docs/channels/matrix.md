...
Matrix is an open, decentralized messaging protocol. FastClaw connects as a Matrix **user**
...
```bash
fastclaw plugins install @fastclaw/matrix
```

Local checkout (when running from a git repo):

```bash
fastclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `fastclaw plugins install @fastclaw/matrix`
   - From a local checkout: `fastclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: FastClaw calls the same
     login endpoint, stores the access token in `~/.fastclaw/credentials/matrix/credentials.json`,
...
- On first connection, FastClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  FastClaw logs a warning.
...
`~/.fastclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `fastclaw pairing list matrix`
  - `fastclaw pairing approve matrix <CODE>`
...
- On startup, FastClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
