...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects FastClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
fastclaw plugins install @fastclaw/feishu
```

Local checkout (when running from a git repo):

```bash
fastclaw plugins install ./extensions/feishu
```
...
If you just installed FastClaw, run the wizard:
...
2. Configuring app credentials in FastClaw
...
1. You already ran `fastclaw channels add` for Feishu
2. The gateway is running (`fastclaw gateway status`)
...
```bash
fastclaw pairing approve feishu <CODE>
```
...
2. Run `fastclaw logs --follow` and look for `chat_id`
...
2. Run `fastclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `fastclaw gateway status`  | Show gateway status           |
| `fastclaw gateway install` | Install/start gateway service |
| `fastclaw gateway stop`    | Stop gateway service          |
| `fastclaw gateway restart` | Restart gateway service       |
| `fastclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `fastclaw logs --follow`
...
5. Ensure the gateway is running: `fastclaw gateway status`
6. Check logs: `fastclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.fastclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.fastclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
