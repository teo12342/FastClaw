...
read_when:
  - Setting up Twitch chat integration for FastClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. FastClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
fastclaw plugins install @fastclaw/twitch
```

Local checkout (when running from a git repo):

```bash
fastclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `FASTCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "fastclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use FASTCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
FASTCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
