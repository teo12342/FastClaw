...
Tlon is a decentralized messenger built on Urbit. FastClaw connects to your Urbit ship and can
...
```bash
fastclaw plugins install @fastclaw/tlon
```

Local checkout (when running from a git repo):

```bash
fastclaw plugins install ./extensions/tlon
```
...
By default, FastClaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `fastclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, FastClaw replies in-thread.
