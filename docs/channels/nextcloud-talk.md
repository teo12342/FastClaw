...
```bash
fastclaw plugins install @fastclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
fastclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "FastClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `fastclaw pairing list nextcloud-talk`
  - `fastclaw pairing approve nextcloud-talk <CODE>`
