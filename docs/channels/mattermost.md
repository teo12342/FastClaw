---
summary: "Mattermost bot setup and FastClaw config"
read_when:
...
```bash
fastclaw plugins install @fastclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
fastclaw plugins install ./extensions/mattermost
```
...
4. Configure FastClaw and start the gateway.
...
- Approve via:
  - `fastclaw pairing list mattermost`
  - `fastclaw pairing approve mattermost <CODE>`
...
Use these target formats with `fastclaw message send` or cron/webhooks:
