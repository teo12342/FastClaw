...
FastClaw supports Kimi as a `web_search` provider, using Moonshot web search
...
<Steps>
  ...
  <Step title="Store the key">
    ...
    ```bash
    fastclaw configure --section web
    ```
...
Gateway environment. For a gateway install, put it in `~/.fastclaw/.env`.
