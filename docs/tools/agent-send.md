---
summary: "Direct `fastclaw agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `fastclaw agent` (direct agent runs)

`fastclaw agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `fastclaw message --target`).
...
```bash
fastclaw agent --to +15555550123 --message "status update"
...
fastclaw agent --to +15555550123 --message "Summon reply" --deliver
