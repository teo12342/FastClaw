...
read_when:
  - Running FastClaw Gateway in WSL2 while Chrome lives on Windows
...
- FastClaw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/fastclaw/fastclaw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- FastClaw and Chrome are on the same machine
...
- FastClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, FastClaw is not the problem yet.
...
Fix that before touching FastClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point FastClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting FastClaw to succeed
...
```bash
fastclaw browser open https://example.com --browser-profile remote
fastclaw browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `fastclaw browser tabs` returns the target
...
3. FastClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug FastClaw config or Control UI auth
