---
title: Lobster
summary: "Typed workflow runtime for FastClaw with resumable approval gates."
description: Typed workflow runtime for FastClaw — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets FastClaw run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: FastClaw runs one Lobster tool call and gets a structured result.
...
FastClaw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | fastclaw.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
fastclaw.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In FastClaw tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the FastClaw Gateway (see the [Lobster repo](https://github.com/fastclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), FastClaw keeps core tools enabled. To restrict core
...
→ fastclaw calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ fastclaw calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls FastClaw tools that do.
