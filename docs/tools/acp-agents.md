...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let FastClaw run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask FastClaw in plain language to "run this in Codex" or "start Claude Code in a thread", FastClaw should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want FastClaw-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | FastClaw native sub-agent runtime  |
...
- FastClaw binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, FastClaw returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, FastClaw uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, FastClaw may default to persistent behavior per runtime path
...
If no target resolves, FastClaw returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, FastClaw returns a clear unsupported-control error.
...
When FastClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal FastClaw `agentId` path).
...
```bash
fastclaw plugins install @fastclaw/acpx
fastclaw config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
fastclaw plugins install ./extensions/acpx
```
...
`@fastclaw/acpx` now enforces a strict plugin-local pinning model:
...
- FastClaw startup stays non-blocking while acpx ensure runs.
