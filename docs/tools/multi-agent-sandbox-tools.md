...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `fastclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `fastclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=fastclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${FASTCLAW_STATE_DIR:-$HOME/.fastclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
