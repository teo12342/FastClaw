---
summary: "Chrome extension: let FastClaw drive your existing Chrome tab"
read_when:
...
The FastClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate fastclaw-managed Chrome profile.
...
FastClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the FastClaw release (npm package) as static files. There is no separate “build” step.

After upgrading FastClaw:

- Re-run `fastclaw browser extension install` to refresh the installed files under your FastClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

FastClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `FASTCLAW_GATEWAY_TOKEN`)
...
- Open the tab you want FastClaw to control.
...
- `ON`: attached; FastClaw can drive that tab.
...
Debugging: `fastclaw sandbox explain`
...
`fastclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `fastclaw browser extension install` first to copy the extension to a stable location under your FastClaw state directory.
...
- **This is not isolated** like the dedicated fastclaw-managed profile.
