...
FastClaw can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere FastClaw can send audio; Telegram gets a round voice-note bubble.
...
FastClaw defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `fastclaw.json`.
...
- If `provider` is **unset**, FastClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.fastclaw/settings/tts.json`, override with `FASTCLAW_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, FastClaw retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, FastClaw:
...
Discord note: `/tts` is a built-in Discord command, so FastClaw registers
...
reply delivery. When the result is Telegram-compatible, FastClaw marks it for
