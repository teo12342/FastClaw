...
FastClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, FastClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `fastclaw configure --section web`. It stores the key in
`~/.fastclaw/fastclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.fastclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
