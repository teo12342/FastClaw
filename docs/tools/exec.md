...
- `gateway`/`node` approvals are controlled by `~/.fastclaw/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  FastClaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`fastclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `fastclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
