...
- **gateway host** → `fastclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.fastclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.fastclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `fastclaw approvals allowlist ...`).
- `fastclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `fastclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.fastclaw/exec-approvals.json` directly.

CLI: `fastclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
