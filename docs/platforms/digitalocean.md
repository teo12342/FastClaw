---
summary: "FastClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up FastClaw on DigitalOcean
  - Looking for cheap VPS hosting for FastClaw
title: "DigitalOcean"
---

# FastClaw on DigitalOcean

## Goal

Run a persistent FastClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status fastclaw-gateway.service

# View logs
journalctl --user -u fastclaw-gateway.service -f
```
...
```bash
fastclaw config set gateway.bind tailnet
fastclaw gateway restart
```
...
```bash
fastclaw channels login whatsapp
# Scan QR code
```
...
- `~/.fastclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u fastclaw --no-pager -n 50
```
