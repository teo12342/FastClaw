...
service via the external `fastclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.fastclaw.gateway`
  (or `ai.fastclaw.<profile>` when using `--profile`/`FASTCLAW_PROFILE`; legacy `com.fastclaw.*` is supported).
