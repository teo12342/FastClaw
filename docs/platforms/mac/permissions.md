...
- Same path: run the app from a fixed location (for FastClaw, `dist/FastClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.fastclaw.mac
sudo tccutil reset ScreenCapture ai.fastclaw.mac
...
Workaround: move files into the FastClaw workspace (`~/.fastclaw/workspace`) if you want to avoid per-folder grants.
