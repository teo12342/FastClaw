---
summary: "FastClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
...
# FastClaw on Raspberry Pi

## Goal

Run a persistent, always-on FastClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most FastClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled fastclaw
...
```bash
# Check logs
journalctl -u fastclaw --no-pager -n 100

# Common fix: rebuild
cd ~/fastclaw  # if using hackable install
npm run build
sudo systemctl restart fastclaw
```
