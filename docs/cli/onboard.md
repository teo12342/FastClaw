---
summary: "CLI reference for `fastclaw onboard` (interactive onboarding wizard)"
read_when:
...
```bash
fastclaw onboard
fastclaw onboard --flow quickstart
fastclaw onboard --flow manual
fastclaw onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `fastclaw dashboard` (Control UI, no channel setup).
