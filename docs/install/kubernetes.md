---
summary: "Deploy FastClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run FastClaw on a Kubernetes cluster
  - You want to test FastClaw in a Kubernetes environment
title: "Kubernetes"
---

# FastClaw on Kubernetes

A minimal starting point for running FastClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

FastClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/fastclaw 18789:18789 -n fastclaw
...
```bash
kubectl get secret fastclaw-secrets -n fastclaw -o jsonpath='{.data.FASTCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: fastclaw (configurable via FASTCLAW_NAMESPACE)
├── Deployment/fastclaw        # Single pod, init container + gateway
├── Service/fastclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/fastclaw-config  # fastclaw.json + AGENTS.md
└── Secret/fastclaw-secrets    # Gateway token + API keys
```
...
Edit `fastclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret fastclaw-secrets -n fastclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/fastclaw -n fastclaw
```

### Custom namespace

```bash
FASTCLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/fastclaw/fastclaw:latest # or pin to a specific version from https://github.com/fastclaw/fastclaw/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # fastclaw.json + AGENTS.md
