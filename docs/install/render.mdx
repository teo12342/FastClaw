...
Deploy FastClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/fastclaw/fastclaw)
...
repository configures everything needed to run FastClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: FASTCLAW_STATE_DIR
        value: /data/.fastclaw
      - key: FASTCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: FASTCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/fastclaw`.
...
If you use the original FastClaw repository, Render will not auto-deploy your FastClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For FastClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any FastClaw host.
