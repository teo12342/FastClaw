...
Deploy FastClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/fastclaw` — Control UI
...
- `FASTCLAW_STATE_DIR=/data/.fastclaw` (recommended)
- `FASTCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `FASTCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your FastClaw state + workspace so you can migrate to another host without losing config or memory.
