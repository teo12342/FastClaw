---
summary: "Host FastClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up FastClaw on Oracle Cloud
  - Looking for free VPS hosting for FastClaw
...
Run a persistent FastClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@fastclaw`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=fastclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `fastclaw doctor --non-interactive` and check logs with `journalctl --user -u fastclaw-gateway -n 50`.
...
- [Updating](/install/updating) -- keep FastClaw up to date
