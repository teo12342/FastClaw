---
summary: "Host FastClaw on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up FastClaw on a Raspberry Pi
  - Running FastClaw on ARM devices
...
Run a persistent, always-on FastClaw Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well.
...
<Steps>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'fastclaw dashboard --no-open'
    ```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/fastclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/fastclaw-compile-cache
mkdir -p /var/tmp/fastclaw-compile-cache
export FASTCLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
**Service will not start** -- Check logs with `journalctl -u fastclaw --no-pager -n 100` and run `fastclaw doctor --non-interactive`.
