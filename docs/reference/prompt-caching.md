...
- With Anthropic API-key auth profiles, FastClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, FastClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
FastClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.fastclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$FASTCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `FASTCLAW_CACHE_TRACE=1` enables cache tracing.
- `FASTCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `FASTCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `FASTCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `FASTCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
