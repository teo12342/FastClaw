...
FastClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- FastClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- FastClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
