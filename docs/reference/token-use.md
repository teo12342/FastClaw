---
summary: "How FastClaw builds prompt context and reports token usage + costs"
read_when:
...
FastClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

FastClaw assembles its own system prompt on every run. It includes:
...
For images, FastClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from FastClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `fastclaw status --usage` and `fastclaw channels list` show
...
`cacheWrite`. If pricing is missing, FastClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. FastClaw can
...
Anthropic's 1M context window is currently beta-gated. FastClaw can inject the
...
FastClaw skips the `context-1m-*` beta header because Anthropic currently
