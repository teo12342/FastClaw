...
- In scope: strictly user-supplied credentials that FastClaw does not mint or rotate.
- Out of scope: runtime-minted or rotating credentials, OAuth refresh material, and session-like artifacts.

## Supported credentials

### `fastclaw.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- Marker persistence is source-authoritative: FastClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
- For web search:
