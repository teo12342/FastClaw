...
FastClaw features that can generate provider usage or paid API calls.
...
- `fastclaw status --usage` and `fastclaw channels list` show provider **usage windows**
...
FastClaw can pick up credentials from:
...
- `fastclaw status --usage`
- `fastclaw models status --json`
...
`fastclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
