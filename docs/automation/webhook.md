...
read_when:
  ...
  - Wiring external systems into FastClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your FastClaw config directory (typically `~/.fastclaw/hooks/transforms`).
...
- `fastclaw webhooks gmail setup` writes `hooks.gmail` config for `fastclaw webhooks gmail run`.
