...
- Jobs persist under `~/.fastclaw/cron/` so restarts don’t lose schedules.
...
fastclaw cron list
fastclaw cron run <job-id>
fastclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.fastclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `fastclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, FastClaw applies a
...
If `delivery` is omitted for isolated jobs, FastClaw defaults to `announce`.
...
- Job store: `~/.fastclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.fastclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, FastClaw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `fastclaw cron runs` and adjust retention before logs become large
...
fastclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
fastclaw cron edit <jobId> --agent ops
fastclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
fastclaw cron run <jobId>
fastclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
fastclaw cron edit <jobId> \
...
```bash
fastclaw cron edit <jobId> --exact
```
...
```bash
fastclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`fastclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `FASTCLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- FastClaw applies exponential retry backoff for recurring jobs after consecutive errors:
