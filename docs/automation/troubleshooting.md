...
fastclaw system heartbeat last
fastclaw logs --follow
fastclaw config get agents.defaults.heartbeat
fastclaw channels status --probe
```
...
```bash
fastclaw config get agents.defaults.heartbeat.activeHours
fastclaw config get agents.defaults.heartbeat.activeHours.timezone
fastclaw config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
fastclaw cron list
fastclaw logs --follow
```
