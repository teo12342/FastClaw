...
summary: "Architecture of FastClaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in FastClaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how FastClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

FastClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, FastClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in FastClaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createFastClawCodingTools()
...
├── fastclaw-tools.ts              # FastClaw-specific tools
...
Image injection is prompt-local: FastClaw loads image refs from the current prompt and
...
2. **Custom Replacements**: FastClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **FastClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures FastClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, FastClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
FastClaw wraps this with `guardSessionManager()` for tool result safety.
...
FastClaw maintains an auth profile store with multiple API keys per provider:
...
FastClaw loads custom pi extensions for specialized behavior:
...
FastClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | FastClaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom FastClaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.fastclaw/agents/<agentId>/sessions/` (or `$FASTCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `FASTCLAW_LIVE_TEST=1`)
