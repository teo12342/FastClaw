...
- During heartbeat runs, FastClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `fastclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, FastClaw skips the heartbeat run entirely (no model call).
...
```bash
fastclaw system event --text "Check for urgent follow-ups" --mode now
```
