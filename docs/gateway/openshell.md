---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for FastClaw agents"
read_when:
...
OpenShell is a managed sandbox backend for FastClaw. Instead of running Docker
containers locally, FastClaw delegates sandbox lifecycle to the `openshell` CLI,
...
- FastClaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, FastClaw creates an OpenShell
...
- Before `exec`, FastClaw syncs the local workspace into the OpenShell sandbox.
- After `exec`, FastClaw syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace
  remains the source of truth between turns.

Best for:

- You edit files locally outside FastClaw and want those changes visible in the
...
- When the sandbox is first created, FastClaw seeds the remote workspace from
...
- FastClaw does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside FastClaw after the initial seed,
the remote sandbox does **not** see those changes. Use
`fastclaw sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"fastclaw"`  | Sandbox source for first-time create                  |
...
1. FastClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. FastClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `fastclaw sandbox` commands
