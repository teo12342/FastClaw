...
summary: "Complete reference for every FastClaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.fastclaw/fastclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — FastClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `fastclaw doctor` into `whatsapp/default`.
...
- FastClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `fastclaw plugins install @fastclaw/mattermost`.
...
FastClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `fastclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, FastClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `fastclaw doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
- `config: true` enables `/config` (reads/writes `fastclaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, FastClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, FastClaw assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.fastclaw/sandboxes`
...
noVNC observer access uses VNC auth by default and FastClaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `fastclaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, FastClaw starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:fastclaw`   | All built-in tools (excludes provider plugins)                                           |
...
FastClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.fastclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `FASTCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `fastclaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `fastclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `fastclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `fastclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `fastclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `fastclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.fastclaw/extensions`, `<workspace>/.fastclaw/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `fastclaw plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or FASTCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `FASTCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.fastclaw-dev` + port `19001`), `--profile <name>` (uses `~/.fastclaw-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-fastclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `FASTCLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or FASTCLAW_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__fastclaw__/a2ui/`.
...
- Hostname defaults to `fastclaw`. Override with `FASTCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.fastclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `fastclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.fastclaw/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.fastclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/fastclaw/fastclaw-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `fastclaw doctor --fix` can strip unknown keys).
