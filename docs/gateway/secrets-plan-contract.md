...
read_when:
  - Generating or reviewing `fastclaw secrets apply` plan files
...
This page defines the strict contract enforced by `fastclaw secrets apply`.

If a target does not match these rules, apply fails before mutating config.

## Plan file shape

`fastclaw secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
If apply fails with an invalid target path message, regenerate the plan with `fastclaw secrets configure` or fix the target path to one of the allowed shapes above.
