...
`fastclaw doctor` is the repair + migration tool for FastClaw. It fixes stale
...
```bash
fastclaw doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
fastclaw doctor --repair --force
```
...
- Extra workspace dir detection (`~/fastclaw`).
...
you to run `fastclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.fastclaw/fastclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.fastclaw/sessions/` to `~/.fastclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.fastclaw/agent/` to `~/.fastclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.fastclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.fastclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `fastclaw doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.fastclaw` folders exist across
  home directories or when `FASTCLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.fastclaw/fastclaw.json` is
...
offers to remove them and install the FastClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named FastClaw gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `fastclaw doctor --generate-gateway-token` to force token
...
- `fastclaw doctor` prompts before rewriting supervisor config.
- `fastclaw doctor --yes` accepts the default repair prompts.
- `fastclaw doctor --repair` applies recommended fixes without prompts.
- `fastclaw doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `fastclaw gateway install --force`.
