...
FastClaw supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, FastClaw performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). FastClaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/FastClaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/fastclaw"],
...
When reload-time activation fails after a healthy state, FastClaw enters degraded secrets state.
...
- plaintext values at rest (`fastclaw.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `fastclaw.json`
...
FastClaw intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, FastClaw runtime no longer depends on plaintext `auth.json`.
