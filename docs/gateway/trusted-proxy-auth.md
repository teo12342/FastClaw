...
read_when:
  - Running FastClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of FastClaw
...
- You run FastClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. FastClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. FastClaw extracts the user identity from the configured header
...
- FastClaw can stay on loopback HTTP behind the proxy.
...
If FastClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://fastclaw.example.com
    to: http://fastclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
fastclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy fastclaw:18789 {
...
location / {
    ...
    proxy_pass http://fastclaw:18789;
...
`fastclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update FastClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `fastclaw security audit` and review findings
