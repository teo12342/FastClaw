...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in FastClaw
...
Running FastClaw via Node with `tsx` fails at startup with:

```
[fastclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `fastclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch fastclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node fastclaw.mjs status` works on Node 25.
