...
read_when:
  - Designing workspace memory (~/.fastclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep FastClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.fastclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.fastclaw/workspace` as canonical human-readable memory.
...
```
~/.fastclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in FastClaw**, but keep a separable core library.

### Why integrate into FastClaw?

- FastClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `fastclaw memory recall "…" --k 25 --since 30d`
  - `fastclaw memory reflect --since 7d`
...
Pragmatic take for `~/.fastclaw/workspace`:
