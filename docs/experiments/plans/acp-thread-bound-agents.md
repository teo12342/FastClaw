...
This plan defines how FastClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- FastClaw core owns ACP control plane concerns
...
FastClaw should not reimplement ACP transport internals in core.
FastClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in FastClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under FastClaw state dir
...
- ACP event stream is normalized to FastClaw reply chunks
...
For the first production backend (`extensions/acpx`), FastClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses FastClaw `sessionKey`)
...
- FastClaw consumes ndjson events from `acpx --format json --json-strict`
