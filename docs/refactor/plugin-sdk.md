...
- Publish as `fastclaw/plugin-sdk` (or export from core under `fastclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `FastClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: FastClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: FastClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: FastClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: FastClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: FastClawConfig): string;
  };
};
```
...
- Introduce `fastclaw/plugin-sdk`.
- Add `api.runtime` to `FastClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `fastclawRuntime: ">=2026.2.0"`).
