...
`fastclaw.invoke --each`) without adding new FastClaw code per workflow.
...
This extension depends on FastClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** FastClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.fastclaw/extensions` as a standalone plugin directory.
