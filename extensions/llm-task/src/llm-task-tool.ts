...
import Ajv from "ajv";
import { resolvePreferredFastClawTmpDir } from "fastclaw/plugin-sdk";
// NOTE: This extension is intended to be bundled with FastClaw.
// When running from source (tests/dev), FastClaw internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { FastClawPluginApi } from "../../../src/plugins/types.js";
...
export function createLlmTaskTool(api: FastClawPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via fastclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredFastClawTmpDir(), "fastclaw-llm-task-"),
        );
