...
import type { FastClawPluginApi, FastClawPluginToolContext } from "./api.js";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<FastClawPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: FastClawPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<FastClawPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as FastClawPluginApi);
