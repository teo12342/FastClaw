...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("fastclaw-diffs-tool-"));
  });
...
function createApi(): FastClawPluginApi {
  return createTestPluginApi({
    ...
    runtime: {} as FastClawPluginApi["runtime"],
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: FastClawPluginToolContext | undefined = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
