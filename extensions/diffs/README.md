...
Read-only diff viewer plugin for **FastClaw** agents.
...
Set plugin-wide defaults in `~/.fastclaw/fastclaw.json`:
...
Before:
FastClaw supports plugins.

After:
FastClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/fastclaw-diffs`).
