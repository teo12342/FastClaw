...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("fastclaw-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("fastclaw-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("fastclaw-openshell-remote-agent-");
