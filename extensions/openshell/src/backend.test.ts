...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from FastClaw config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "fastclaw-session-1234",
        backendId: "openshell",
        runtimeLabel: "fastclaw-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "fastclaw-session-1234",
        config: expect.objectContaining({
          from: "custom-source",
        }),
      }),
      args: ["sandbox", "get", "fastclaw-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "fastclaw-session-5678",
        backendId: "openshell",
        runtimeLabel: "fastclaw-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "fastclaw",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "fastclaw-session-5678",
        config: expect.objectContaining({
          command: "/usr/local/bin/openshell",
          gateway: "lab",
        }),
      }),
      args: ["sandbox", "delete", "fastclaw-session-5678"],
    });
  });
});
