...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["fastclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["fastclaw", "pollId"]) ??
    readNestedString(value, ["fastclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "fastclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "fastclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          fastclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "fastclaw poll vote",
          displayText: "Vote recorded",
          value: { fastclawPollId: pollId, pollId },
        },
      },
    ],
  };
