...
export const signalDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: FastClawConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "FastClaw"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("fastclaw gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
