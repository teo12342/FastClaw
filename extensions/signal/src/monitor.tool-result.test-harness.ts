...
vi.mock("fastclaw/plugin-sdk/config-runtime", async () => {
  ...
  return {
    ...actual,
    loadConfig: () => config,
    resolveStorePath: vi.fn(() => "/tmp/fastclaw-sessions.json"),
...
vi.mock("fastclaw/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/security-runtime")>(
    "fastclaw/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("fastclaw/plugin-sdk/reply-runtime"),
      import("fastclaw/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
