...
vi.mock("fastclaw/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk")>("fastclaw/plugin-sdk");
  return {
    ...actual,
...
import type { FastClawPluginServiceContext } from "fastclaw/plugin-sdk";
import { emitDiagnosticEvent } from "fastclaw/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/fastclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): FastClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("fastclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("fastclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("fastclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("fastclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("fastclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("fastclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("fastclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("fastclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("fastclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("fastclaw.webhook.processed");
    expect(spanNames).toContain("fastclaw.message.processed");
    expect(spanNames).toContain("fastclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["fastclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("fastclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "fastclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["fastclaw.reason"]).toBe("string");
    expect(String(attrs?.["fastclaw.reason"])).not.toContain(
