...
metadata: { "fastclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == FastClaw `sessions_spawn`
- **File I/O** == FastClaw `read`/`write`
- **Remote fetch** == FastClaw `web_fetch` (or `exec` with curl when POST is required)
