import { DEFAULT_ACCOUNT_ID } from "fastclaw/plugin-sdk/setup";
...
async function runPrepare(cfg: FastClawConfig, accountId: string) {
...
async function runFinalize(cfg: FastClawConfig, accountId: string) {
...
function expectPreparedResult(
...
): { cfg: FastClawConfig } & Exclude<Awaited<ReturnType<typeof runPrepare>>, void | undefined> {
  ...
  return prepared as { cfg: FastClawConfig } & Exclude<
...
describe("telegramSetupWizard.finalize", () => {
  it("shows global config commands for the default account", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining('fastclaw config set channels.telegram.dmPolicy "allowlist"'),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(`fastclaw config set channels.telegram.allowFrom '["YOUR_USER_ID"]'`),
      "Telegram DM access warning",
    );
  });
  ...
  it("shows account-scoped config commands for named accounts", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        'fastclaw config set channels.telegram.accounts.alerts.dmPolicy "allowlist"',
      ),
      "Telegram DM access warning",
    );
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining(
        `fastclaw config set channels.telegram.accounts.alerts.allowFrom '["YOUR_USER_ID"]'`,
      ),
      "Telegram DM access warning",
    );
  });
