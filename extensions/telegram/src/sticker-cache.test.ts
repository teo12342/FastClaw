...
vi.mock("fastclaw/plugin-sdk/media-runtime", () => ({
  AUTO_IMAGE_KEY_PROVIDERS: ["openai"],
  DEFAULT_IMAGE_MODELS: { openai: "gpt-4.1-mini" },
  resolveAutoImageModel: vi.fn(async () => null),
}));

vi.mock("fastclaw/plugin-sdk/media-understanding-runtime", () => ({
  describeImageFileWithModel: vi.fn(),
}));

const TEST_CACHE_DIR = "/tmp/fastclaw-test-sticker-cache/telegram";
...
describe("sticker-cache", () => {
  beforeEach(async () => {
    process.env.FASTCLAW_STATE_DIR = "/tmp/fastclaw-test-sticker-cache";
    fs.rmSync("/tmp/fastclaw-test-sticker-cache", { recursive: true, force: true });
