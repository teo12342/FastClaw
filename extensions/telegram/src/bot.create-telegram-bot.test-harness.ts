import { resolveDefaultModelForAgent } from "fastclaw/plugin-sdk/agent-runtime";
import type { FastClawConfig } from "fastclaw/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "fastclaw/plugin-sdk/reply-runtime";
import type { MsgContext } from "fastclaw/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "fastclaw/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "fastclaw/plugin-sdk/reply-runtime";
import type { MockFn } from "fastclaw/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("fastclaw/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("fastclaw/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("fastclaw/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/fastclaw-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("fastclaw/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("fastclaw/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("fastclaw/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("fastclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("fastclaw/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: FastClawConfig): {
...
vi.doMock("fastclaw/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("fastclaw/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("fastclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("fastclaw/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("fastclaw/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: FastClawConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: FastClawConfig) => {
