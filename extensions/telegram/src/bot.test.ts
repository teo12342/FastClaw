...
import type { PluginInteractiveTelegramHandlerContext } from "fastclaw/plugin-sdk/core";
...
describe("createTelegramBot", () => {
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/fastclaw-telegram-model-compact-${process.pid}-${Date.now()}.json`;
    const config = {
      agents: {
        defaults: {
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/fastclaw-telegram-model-default-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "FastClaw" },
        },
      },
      me: { id: 999, username: "fastclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
