...
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "fastclaw/plugin-sdk/setup";
...
function shouldShowTelegramDmAccessWarning(cfg: FastClawConfig, accountId: string): boolean {
...
function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`fastclaw config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`fastclaw config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
