...
function ensureZalouserPluginEnabled(cfg: FastClawConfig): FastClawConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("fastclaw directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
  ...
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ...
        : resolveDefaultZalouserAccountId(cfg as FastClawConfig);
    return await promptZalouserAllowFrom({
      cfg: cfg as FastClawConfig,
      prompter,
      accountId: id,
    });
  },
...
export const zalouserSetupWizard: ChannelSetupWizard = {
  ...
  groupAccess: {
    ...
    setPolicy: ({ cfg, accountId, policy }) =>
      setZalouserGroupPolicy(cfg as FastClawConfig, accountId, policy),
    resolveAllowlist: async ({ cfg, accountId, entries, prompter }) => {
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("fastclaw directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
      ...
      const updatedAccount = resolveZalouserAccountSync({ cfg: cfg as FastClawConfig, accountId });
      try {
    ...
    applyAllowlist: ({ cfg, accountId, resolved }) =>
      setZalouserGroupAllowlist(cfg as FastClawConfig, accountId, resolved as string[]),
  },
