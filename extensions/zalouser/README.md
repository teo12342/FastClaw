...
FastClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
fastclaw channels login --channel zalouser
...
```bash
fastclaw message send --channel zalouser --target <threadId> --message "Hello from FastClaw!"
```
...
```bash
fastclaw channels login --channel zalouser              # Login via QR
fastclaw channels login --channel zalouser --account work
fastclaw channels status --probe
fastclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
fastclaw directory self --channel zalouser
fastclaw directory peers list --channel zalouser --query "name"
fastclaw directory groups list --channel zalouser --query "work"
fastclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
fastclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
fastclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `fastclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
fastclaw channels login --channel zalouser --account work
fastclaw message send --channel zalouser --account work --target <id> --message "Hello"
