...
export const imessageDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: FastClawConfig) => cfg.channels?.imessage?.dmPolicy ?? "pairing",
  setPolicy: (cfg: FastClawConfig, policy) =>
    setChannelDmPolicyWithAllowFrom({
      cfg,
      channel,
      dmPolicy: policy,
...
function resolveIMessageCliPath(params: { cfg: FastClawConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    ...
    "Ensure FastClaw has Full Disk Access to Messages DB.",
