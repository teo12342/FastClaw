...
import { generateSecureUuid } from "fastclaw/plugin-sdk/infra-runtime";
...
import { getChildLogger } from "fastclaw/plugin-sdk/text-runtime";
...
import { markdownToWhatsApp } from "fastclaw/plugin-sdk/text-runtime";
import { toWhatsappJid } from "fastclaw/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: FastClawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
