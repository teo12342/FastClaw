...
const resolveWhatsAppAuthDirMock = vi.hoisted(() =>
  vi.fn(() => ({
    authDir: "/tmp/fastclaw-whatsapp-test",
  })),
);
...
vi.mock("fastclaw/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/setup")>(
    "fastclaw/plugin-sdk/setup",
  );
  return {
    ...actual,
...
describe("whatsapp setup wizard", () => {
  beforeEach(async () => {
    ...
    resolveWhatsAppAuthDirMock.mockReturnValue({ authDir: "/tmp/fastclaw-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("fastclaw channels login"),
      "WhatsApp",
    );
  });
