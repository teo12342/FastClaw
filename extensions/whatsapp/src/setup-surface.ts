...
import type { ChannelSetupWizard } from "fastclaw/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "fastclaw/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: FastClawConfig,
  patch: Partial<NonNullable<NonNullable<FastClawConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: FastClawConfig, dmPolicy: DmPolicy): FastClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: FastClawConfig, allowFrom?: string[]): FastClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: FastClawConfig, selfChatMode: boolean): FastClawConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: FastClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so FastClaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for FastClaw" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("fastclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
