...
import { resolvePreferredFastClawTmpDir } from "../../../src/infra/tmp-fastclaw-dir.js";
...
vi.mock("fastclaw/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/media-runtime")>(
    "fastclaw/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
    convertHeicToJpeg: (...args: unknown[]) => convertHeicToJpegMock(...args),
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredFastClawTmpDir(), "fastclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub FASTCLAW_STATE_DIR.
    // Also keep it outside the FastClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["FASTCLAW_STATE_DIR"]);
    process.env.FASTCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "fastclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredFastClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default FastClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default FastClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
