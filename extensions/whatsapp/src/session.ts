...
import { VERSION } from "fastclaw/plugin-sdk/cli-runtime";
import { danger, success } from "fastclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "fastclaw/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "fastclaw/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("fastclaw channels login")}`,
              ),
            );
          }
        }
