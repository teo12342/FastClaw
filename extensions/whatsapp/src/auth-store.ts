...
import { info, success } from "fastclaw/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "fastclaw/plugin-sdk/state-paths";
import type { WebChannel } from "fastclaw/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "fastclaw/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("fastclaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
