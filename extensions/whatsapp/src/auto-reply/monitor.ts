import { resolveInboundDebounceMs } from "fastclaw/plugin-sdk/channel-inbound";
import { formatCliCommand } from "fastclaw/plugin-sdk/cli-runtime";
import { waitForever } from "fastclaw/plugin-sdk/cli-runtime";
import { hasControlCommand } from "fastclaw/plugin-sdk/command-auth";
import { loadConfig } from "fastclaw/plugin-sdk/config-runtime";
import { formatDurationPrecise } from "fastclaw/plugin-sdk/infra-runtime";
import { enqueueSystemEvent } from "fastclaw/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "fastclaw/plugin-sdk/reply-history";
import { getReplyFromConfig } from "fastclaw/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "fastclaw/plugin-sdk/routing";
import { logVerbose } from "fastclaw/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "fastclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "fastclaw/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "fastclaw/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("fastclaw channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("fastclaw channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
