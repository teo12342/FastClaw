...
describe("web processMessage inbound context", () => {
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-process-message-"));
  ...
  it("does not force an [fastclaw] response prefix in self-chats when identity is unset", async () => {
    await processSelfDirectMessage({
      messages: {},
      session: { store: sessionStorePath },
    } as unknown as ReturnType<typeof import("../../../../../src/config/config.js").loadConfig>);
