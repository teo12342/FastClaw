...
vi.mock("fastclaw/plugin-sdk/agent-runtime", () => ({
...
vi.mock("fastclaw/plugin-sdk/routing", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/routing")>();
  return {
    ...actual,
...
vi.mock("fastclaw/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/runtime-env")>();
  const logger = {
    child: () => logger,
...
vi.mock("fastclaw/plugin-sdk/text-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/text-runtime")>();
  return {
    ...actual,
    redactIdentifier,
  };
...
vi.mock("../auth-store.js", () => ({
  WA_WEB_AUTH_DIR: "/tmp/fastclaw-oauth/whatsapp/default",
  resolveDefaultWebAuthDir: () => "/tmp/fastclaw-oauth/whatsapp/default",
