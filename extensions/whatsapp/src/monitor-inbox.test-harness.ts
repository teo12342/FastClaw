...
import { resetLogger, setLoggerOverride } from "fastclaw/plugin-sdk/runtime-env";
...
vi.mock("fastclaw/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/media-runtime")>();
  return {
    ...actual,
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "fastclaw-auth-"));
    } else {
      authDir = undefined;
    }
  });
