...
import { parseDurationMs } from "fastclaw/plugin-sdk/cli-runtime";
import {
  definePluginEntry,
...
import { normalizeModelCompat } from "fastclaw/plugin-sdk/provider-models";
import { fetchClaudeUsage } from "fastclaw/plugin-sdk/provider-usage";
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("fastclaw doctor --yes")}"`,
  ].join("\n");
}
