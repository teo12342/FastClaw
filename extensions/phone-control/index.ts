...
import type { FastClawPluginApi, FastClawPluginService } from "fastclaw/plugin-sdk";
...
function normalizeDenyList(cfg: FastClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: FastClawPluginApi["config"]): string[] {
...
export default function register(api: FastClawPluginApi) {
  ...
  const timerService: FastClawPluginService = {
