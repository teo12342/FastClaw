...
function getChannelConfig(cfg: FastClawConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: FastClawConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: FastClawConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: FastClawConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL FastClaw uses to send replies back to Chat.",
      ],
