...
import { normalizeSecretInput } from "fastclaw/plugin-sdk/provider-auth";
import { normalizeResolvedSecretInputString } from "fastclaw/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<FastClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<FastClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: FastClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: FastClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: FastClawConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: FastClawConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: FastClawConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: FastClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: FastClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: FastClawConfig, override?: number): number {
