...
import { formatCliCommand } from "fastclaw/plugin-sdk/setup-tools";
...
export async function refreshQwenPortalCredentials(
  ...
  if (!response.ok) {
    ...
    if (response.status === 400) {
      throw new Error(
        `Qwen OAuth refresh token expired or invalid. Re-authenticate with \`${formatCliCommand("fastclaw models auth login --provider qwen-portal")}\`.`,
      );
    }
