...
import type { FastClawPluginApi, FastClawPluginToolContext } from "../../../src/plugins/types.js";
import {
...
function fakeApi(overrides: Partial<FastClawPluginApi> = {}): FastClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<FastClawPluginToolContext> = {}): FastClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: FastClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
