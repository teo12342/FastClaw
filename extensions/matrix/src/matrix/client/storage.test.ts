...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-fastclaw-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-matrix-storage-"));
    const stateDir = path.join(homeDir, ".fastclaw");
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      FASTCLAW_HOME: path.dirname(stateDir),
      FASTCLAW_STATE_DIR: stateDir,
      FASTCLAW_TEST_FAST: "1",
