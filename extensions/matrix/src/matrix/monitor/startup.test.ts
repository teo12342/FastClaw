...
const hoisted = vi.hoisted(() => ({
  ...
  summarizeMatrixDeviceHealth: vi.fn(() => ({
    staleFastClawDevices: [] as Array<{ deviceId: string }>,
  })),
...
describe("runMatrixStartupMaintenance", () => {
  beforeEach(() => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockClear().mockReturnValue({ staleFastClawDevices: [] });
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockReturnValue({
      staleFastClawDevices: [{ deviceId: "DEV123" }],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale FastClaw devices detected for @bot:example.org: DEV123. Run 'fastclaw matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'fastclaw matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
