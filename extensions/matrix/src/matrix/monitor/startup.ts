...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleFastClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale FastClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleFastClawDevices.map((device) => device.deviceId).join(", ")}. Run 'fastclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
