...
import { isFastClawManagedMatrixDevice } from "./matrix/device-health.js";
import {
...
function formatMatrixCliCommand(command: string, accountId?: string): string {
  ...
  return `fastclaw matrix ${command}${suffix}`;
}
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleFastClawDeviceIds: addedDevices
        .filter((device) => !device.current && isFastClawManagedMatrixDevice(device.displayName))
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.fastclaw.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleFastClawDeviceIds.length > 0) {
              console.log(
                `Matrix device hygiene warning: stale FastClaw devices detected (${result.deviceHealth.staleFastClawDeviceIds.join(", ")}). Run 'fastclaw matrix devices prune-stale --account ${result.accountId}'.`,
              );
            }
            ...
            const bindHint = `fastclaw agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale FastClaw-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
      ...
      await runMatrixCliCommand({
        ...
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale FastClaw devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.join(", ") : "none"}`,
          );
