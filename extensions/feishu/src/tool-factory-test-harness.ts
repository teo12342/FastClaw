import type { AnyAgentTool, FastClawPluginApi } from "fastclaw/plugin-sdk";
...
export function createToolFactoryHarness(cfg: FastClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<FastClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
