...
import type { FastClawConfig, PluginRuntime } from "fastclaw/plugin-sdk";
import type { DynamicAgentCreationConfig } from "./types.js";

export type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: FastClawConfig;
  agentId?: string;
};
...
export async function maybeCreateDynamicAgent(params: {
  ...
  if (existingAgent) {
    ...
    const updatedCfg: FastClawConfig = {
      ...cfg,
      bindings: [
        ...existingBindings,
        {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.fastclaw/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.fastclaw/agents/{agentId}/agent";
