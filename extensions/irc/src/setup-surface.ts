...
import type { ChannelSetupDmPolicy } from "fastclaw/plugin-sdk/setup";
import type { ChannelSetupWizard } from "fastclaw/plugin-sdk/setup";
import { formatDocsLink } from "fastclaw/plugin-sdk/setup";
import type { WizardPrompter } from "fastclaw/plugin-sdk/setup";
...
export const ircSetupWizard: ChannelSetupWizard = {
  ...
  textInputs: [
    ...
    },
    {
      ...
      initialValue: ({ cfg, accountId }) =>
        resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.realname || "FastClaw",
      validate: ({ value }) => (String(value ?? "").trim() ? undefined : "Required"),
      normalizeValue: ({ value }) => String(value).trim(),
      applySet: async ({ cfg, accountId, value }) =>
        updateIrcAccountConfig(cfg as CoreConfig, accountId, {
  ...
  completionNote: {
    ...
    lines: [
      ...
      "Command: fastclaw channels status --probe",
      `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
    ],
  },
