/**
 * Twitch channel plugin for FastClaw.
 *
...
import { buildChannelConfigSchema } from "fastclaw/plugin-sdk";
 ...
 * for FastClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: FastClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: FastClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: FastClawConfig): boolean => {
