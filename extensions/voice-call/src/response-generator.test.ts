...
function createAgentRuntime(payloads: Array<Record<string, unknown>>) {
  ...
  const runtime = {
    ...
    resolveAgentDir: () => "/tmp/fastclaw/agents/main",
    resolveAgentWorkspaceDir: () => "/tmp/fastclaw/workspace/main",
    ...
    runEmbeddedPiAgent,
    session: {
      resolveStorePath: () => "/tmp/fastclaw/sessions.json",
      loadSessionStore: () => ({}),
      saveSessionStore: async () => {},
      resolveSessionFilePath: () => "/tmp/fastclaw/sessions/session.jsonl",
    },
