...
function resolveFastClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.FASTCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set FASTCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveFastClawRoot(), "dist", "extensionAPI.js");
