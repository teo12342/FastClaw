import { describeAccountSnapshot } from "fastclaw/plugin-sdk/account-helpers";
import { formatAllowFromLowercase } from "fastclaw/plugin-sdk/allow-from";
import {
  adaptScopedAccountAccessor,
  createScopedChannelConfigAdapter,
} from "fastclaw/plugin-sdk/channel-config-helpers";
import { createChannelPluginBase } from "fastclaw/plugin-sdk/core";
import {
  formatDocsLink,
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "FastClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for FastClaw`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "FastClaw"): string[] {
  return [
