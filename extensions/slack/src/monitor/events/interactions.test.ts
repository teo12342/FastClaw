...
vi.mock("fastclaw/plugin-sdk/infra-runtime", async (importOriginal) => {
...
vi.mock("fastclaw/plugin-sdk/plugin-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/plugin-runtime")>();
  return {
    ...actual,
...
vi.mock("fastclaw/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/conversation-runtime")>(
    "fastclaw/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
describe("registerSlackInteractionEvents", () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "fastclaw:verify" }],
            },
          ],
        },
      },
  ...
  it("registers a matcher that accepts plugin action ids beyond the FastClaw prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher).toBeTruthy();
    expect(matcher?.test("fastclaw:verify")).toBe(true);
  ...
  it("resolves plugin binding approvals from shared interactive Slack actions", async () => {
    ...
    await handler!({
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "fastclaw:reply_button" }],
            },
          ],
        },
      },
      action: {
        type: "button",
        action_id: "fastclaw:reply_button",
        block_id: "bind_actions",
  ...
  it("drops modal lifecycle payloads when mismatch guard triggers", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V123",
          callback_id: "fastclaw:deploy_form",
          ...
          blocks: [
            {
              ...
              elements: [{ type: "multi_static_select", action_id: "fastclaw:multi" }],
            },
          ],
        },
      },
          ...
          blocks: [
            {
              type: "actions",
              block_id: "date_block",
              elements: [{ type: "datepicker", action_id: "fastclaw:date" }],
            },
            {
              type: "actions",
              block_id: "time_block",
              elements: [{ type: "timepicker", action_id: "fastclaw:time" }],
            },
            {
              type: "actions",
              block_id: "datetime_block",
              elements: [{ type: "datetimepicker", action_id: "fastclaw:datetime" }],
            },
          ],
        },
      },
  ...
  it("captures modal submissions and enqueues view submission event", async () => {
    ...
    expect(payload).toMatchObject({
      ...
      actionId: "view:fastclaw:deploy_form",
      callbackId: "fastclaw:deploy_form",
      viewId: "V123",
      userId: "U777",
  ...
  it("captures modal input labels and picker values across block types", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V400",
          callback_id: "fastclaw:routing_form",
            ...
            values: {
              ...
              email_block: {
                email_input: {
                  ...
                  value: "team@fastclaw.ai",
                },
              },
              url_block: {
                url_input: {
                  type: "url_text_input",
                  value: "https://docs.fastclaw.ai",
                },
              },
              richtext_block: {
                richtext_input: {
    ...
    expect(payload.inputs).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          inputKind: "email",
          inputEmail: "team@fastclaw.ai",
        }),
        expect.objectContaining({
          actionId: "url_input",
          inputKind: "url",
          inputUrl: "https://docs.fastclaw.ai/",
        }),
        expect.objectContaining({
  ...
  it("truncates rich text preview to keep payload summaries compact", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V555",
          callback_id: "fastclaw:long_richtext",
