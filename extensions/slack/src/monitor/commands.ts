import type { SlackSlashCommandConfig } from "fastclaw/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "fastclaw");
  const name = normalizedName || "fastclaw";
  return {
