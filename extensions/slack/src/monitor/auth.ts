import { readStoreAllowFromForDmPolicy } from "fastclaw/plugin-sdk/security-runtime";
import {
  allowListMatches,
  normalizeAllowList,
...
function getPairingAllowFromCacheTtlMs(): number {
  const raw = process.env.FASTCLAW_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS?.trim();
  if (!raw) {
