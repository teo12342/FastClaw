...
describe("resolveSlackEffectiveAllowFrom", () => {
  const prevTtl = process.env.FASTCLAW_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS;
  ...
  beforeEach(async () => {
    ...
    if (prevTtl === undefined) {
      delete process.env.FASTCLAW_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS;
    } else {
      process.env.FASTCLAW_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS = prevTtl;
    }
  });
  ...
  it("refreshes pairing-store allowFrom when cache TTL is zero", async () => {
    process.env.FASTCLAW_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS = "0";
    readStoreAllowFromForDmPolicyMock.mockResolvedValue(["u2"]);
    const ctx = makeSlackCtx(["u1"]);
