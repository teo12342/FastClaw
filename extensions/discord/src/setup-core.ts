import { DEFAULT_ACCOUNT_ID } from "fastclaw/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "fastclaw/plugin-sdk/config-runtime";
import type { FastClawConfig } from "fastclaw/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "fastclaw/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "fastclaw/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "fastclaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "fastclaw/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: FastClawConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: FastClawConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: FastClawConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: FastClawConfig; accountId: string }) =>
    ...
    disable: (cfg: FastClawConfig) => setSetupChannelEnabled(cfg, channel, false),
