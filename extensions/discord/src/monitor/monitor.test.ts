...
import { buildPluginBindingApprovalCustomId } from "fastclaw/plugin-sdk/conversation-runtime";
...
vi.mock("fastclaw/plugin-sdk/security-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/security-runtime")>();
  return {
    ...actual,
...
vi.mock("fastclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
...
vi.mock("fastclaw/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    upsertChannelPairingRequest: (...args: unknown[]) => upsertPairingRequestMock(...args),
...
vi.mock("fastclaw/plugin-sdk/infra-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("fastclaw/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
    enqueueSystemEvent: (...args: unknown[]) => enqueueSystemEventMock(...args),
...
vi.mock("fastclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.mock("fastclaw/plugin-sdk/config-runtime", async (importOriginal) => {
...
describe("discord component interactions", () => {
  ...
  const createCfg = (): FastClawConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/fastclaw-sessions-test.json");
    ...
    resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
      ...
      binding: {
        ...
        pluginName: "FastClaw App Server",
