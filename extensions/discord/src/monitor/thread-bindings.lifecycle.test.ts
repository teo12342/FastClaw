...
const acpRuntime = await import("fastclaw/plugin-sdk/acp-runtime");
...
describe("thread binding lifecycle", () => {
  ...
  it("persists touched activity timestamps across restart when persistence is enabled", async () => {
    vi.useFakeTimers();
    const previousStateDir = process.env.FASTCLAW_STATE_DIR;
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-thread-bindings-"));
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.FASTCLAW_STATE_DIR = previousStateDir;
      }
  ...
  it("binds current Discord DMs as direct conversation bindings", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:fastclaw-codex-app-server:dm",
      targetKind: "session",
      conversation: {
        channel: "discord",
        accountId: "default",
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/fastclaw-app-server",
      },
    });
