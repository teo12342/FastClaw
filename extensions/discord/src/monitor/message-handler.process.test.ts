...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(() => "/tmp/fastclaw-discord-process-test-sessions.json"),
}));
...
beforeEach(async () => {
  ...
  resolveStorePath.mockReturnValue("/tmp/fastclaw-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/fastclaw-discord-process-test-sessions.json" },
      },
    });
