...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): FastClawConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("fastclaw/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/plugin-runtime")>(
