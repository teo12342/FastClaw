# Google Gemini CLI Auth (FastClaw plugin)
...
```bash
fastclaw plugins enable google-gemini-cli-auth
```
...
```bash
fastclaw models auth login --provider google-gemini-cli --set-default
```
...
- `FASTCLAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `FASTCLAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
