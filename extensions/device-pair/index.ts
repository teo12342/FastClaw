...
function resolveGatewayPort(cfg: FastClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.FASTCLAW_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: FastClawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: FastClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the FastClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the FastClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
