...
import type { FastClawPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredFastClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "fastclaw-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredFastClawTmpDir: pluginApiMocks.resolvePreferredFastClawTmpDir,
...
function createApi(params?: {
  runtime?: FastClawPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as FastClawPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: FastClawPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as FastClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredFastClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredFastClawTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the FastClaw iOS app:");
    expect(text).toContain("![FastClaw pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the FastClaw iOS app:");
