---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for FastClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, FastClaw cron scheduling for periodic checks, or version status checks on a machine running FastClaw (laptop, workstation, Pi, VPS).
---

# FastClaw Host Hardening

## Overview

Assess and harden the host running FastClaw, then align it to a user-defined risk tolerance without breaking access. Use FastClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim FastClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. FastClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if FastClaw can access sensitive data.
...
### 2) Run FastClaw security audits (read-only)

As part of the default read-only checks, run `fastclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `fastclaw security audit` (faster, non-probing)
2. `fastclaw security audit --json` (structured output)

Offer to apply FastClaw safe defaults (numbered):

1. `fastclaw security audit --fix`

Be explicit that `--fix` only tightens FastClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check FastClaw version/update status (read-only)

As part of the default read-only checks, run `fastclaw update status`.
...
- Credential hygiene notes (location of FastClaw creds, prefer disk encryption)
...
- FastClaw security audit (re-run)
...
After FastClaw install or first hardening pass, run at least one baseline audit and version check:

- `fastclaw security audit`
- `fastclaw security audit --deep`
- `fastclaw update status`

Ongoing monitoring is recommended. Use the FastClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `fastclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `fastclaw update status`
...
Before creating, `fastclaw cron list` and match on exact `name`. If found, `fastclaw cron edit <id> ...`.
If not found, `fastclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `fastclaw update status` (preferred for source checkouts and channels)
2. `npm view fastclaw version` (published npm version)

## FastClaw command accuracy

Use only supported commands and flags:

- `fastclaw security audit [--deep] [--fix] [--json]`
- `fastclaw status` / `fastclaw status --deep`
- `fastclaw health --json`
- `fastclaw update status`
- `fastclaw cron add|list|runs|run`

Do not invent CLI flags or imply FastClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by FastClaw compaction:
