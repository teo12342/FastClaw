---
name: node-connect
description: Diagnose FastClaw node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify FastClaw is advertising that route, then fix pairing/auth.
...
- whether `fastclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `fastclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
fastclaw config get gateway.mode
fastclaw config get gateway.bind
fastclaw config get gateway.tailscale.mode
fastclaw config get gateway.remote.url
fastclaw config get gateway.auth.mode
fastclaw config get gateway.auth.allowTailscale
fastclaw config get plugins.entries.device-pair.config.publicUrl
fastclaw qr --json
fastclaw devices list
fastclaw nodes status
```

If this FastClaw instance is pointed at a remote gateway, also run:

```bash
fastclaw qr --remote --json
```
...
`fastclaw qr --json` success means:
...
If `fastclaw qr --json` says `Gateway is only bound to loopback`:
...
- `fastclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run fastclaw qr again, rescan, then approve the pending device pairing.`
