import type { FastClawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "fastclaw-bundled": {
    ...
    canOverride: ["fastclaw-bundled"],
    canBeOverriddenBy: ["fastclaw-managed", "fastclaw-plugin"],
  },
  "fastclaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["fastclaw-bundled", "fastclaw-plugin"],
    canBeOverriddenBy: ["fastclaw-managed"],
  },
  "fastclaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["fastclaw-bundled", "fastclaw-managed", "fastclaw-plugin"],
    canBeOverriddenBy: ["fastclaw-managed"],
  },
  "fastclaw-workspace": {
    precedence: 40,
    ...
    canOverride: ["fastclaw-workspace"],
    canBeOverriddenBy: ["fastclaw-workspace"],
  },
};
...
export function resolveHookConfig(
  config: FastClawConfig | undefined,
  hookKey: string,
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "fastclaw-plugin") {
    return { enabled: true };
  }
