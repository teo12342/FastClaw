...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "fastclaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.FASTCLAW_BUNDLED_HOOKS_DIR;
    process.env.FASTCLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.FASTCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.FASTCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".fastclaw", "extensions", "sample-bundle");
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"fastclaw":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): FastClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("fastclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as FastClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".fastclaw", "extensions", "claude-bundle");
