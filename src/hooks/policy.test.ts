...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "fastclaw-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "fastclaw-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "fastclaw-bundled");
      const plugin = makeHookEntry("shared", "fastclaw-plugin");
      const managed = makeHookEntry("shared", "fastclaw-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("fastclaw-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "fastclaw-managed");
      const workspace = makeHookEntry("shared", "fastclaw-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "fastclaw-workspace");
      const second = makeHookEntry("shared", "fastclaw-workspace");
      second.hook.handlerPath = "/tmp/fastclaw-workspace/shared/handler-2.js";
