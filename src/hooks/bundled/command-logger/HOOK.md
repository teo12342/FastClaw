...
homepage: https://docs.fastclaw.ai/automation/hooks#command-logger
metadata:
  {
    "fastclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.fastclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.fastclaw/logs/commands.log ~/.fastclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/fastclaw`:
   ```
   /home/username/.fastclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
