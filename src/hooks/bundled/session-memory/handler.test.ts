...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => FastClawConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("fastclaw-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): FastClawConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
