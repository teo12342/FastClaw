...
This directory contains hooks that ship with FastClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.fastclaw/fastclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'fastclaw.*gateway' && pnpm fastclaw gateway`
3. Enable the hook: `fastclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.fastclaw.ai/automation/hooks
