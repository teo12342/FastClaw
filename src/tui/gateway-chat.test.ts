...
describe("resolveGatewayConnection", () => {
  ...
  it("uses FASTCLAW_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ FASTCLAW_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers FASTCLAW_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ FASTCLAW_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
