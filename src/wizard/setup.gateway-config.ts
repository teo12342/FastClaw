...
import type { GatewayBindMode, GatewayTailscaleMode, FastClawConfig } from "../config/config.js";
    ...
    if (tokenMode === "ref") {
      if (flow === "quickstart" && quickstartTokenRef) {
        ...
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "FASTCLAW_GATEWAY_TOKEN",
          copy: {
            sourceMessage: "Where is this gateway token stored?",
            envVarPlaceholder: "FASTCLAW_GATEWAY_TOKEN",
          },
        });
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.FASTCLAW_GATEWAY_TOKEN)) ||
        randomToken();
      ...
      const tokenInput = await prompter.text({
        ...
        initialValue:
          ...
          normalizeGatewayTokenInput(process.env.FASTCLAW_GATEWAY_TOKEN) ??
          "",
      });
    ...
    if (!password) {
      const selectedMode = await resolveSecretInputModeForEnvSelection({
        ...
        copy: {
          ...
          plaintextHint: "Stores the password directly in FastClaw config",
        },
      });
      if (selectedMode === "ref") {
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "FASTCLAW_GATEWAY_PASSWORD",
          copy: {
            sourceMessage: "Where is this gateway password stored?",
            envVarPlaceholder: "FASTCLAW_GATEWAY_PASSWORD",
          },
        });
