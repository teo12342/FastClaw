...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "FastClaw is a hobby project and still in beta. Expect sharp edges.",
      "By default, FastClaw is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "FastClaw is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run FastClaw.",
      ...
      "",
      "Must read: https://docs.fastclaw.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("FastClaw setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: FastClawConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("fastclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("fastclaw doctor")}`,
        `Inspect: ${formatCliCommand("fastclaw plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("fastclaw configure")}.`;
  ...
  let localGatewayToken = process.env.FASTCLAW_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.FASTCLAW_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: FastClawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
