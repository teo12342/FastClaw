...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: FastClawConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: FastClawConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: FastClawConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe and TUI", async () => {
    ...
    process.env.FASTCLAW_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
