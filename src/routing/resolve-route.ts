...
function listAgents(cfg: FastClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: FastClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: FastClawConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<FastClawConfig, EvaluatedBindingsCache>();
