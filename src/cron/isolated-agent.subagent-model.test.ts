...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "fastclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".fastclaw", "sessions");
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<FastClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<FastClawConfig>,
