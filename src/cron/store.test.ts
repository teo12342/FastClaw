...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses FASTCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("FASTCLAW_HOME", "/srv/fastclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/fastclaw-home", "cron", "jobs.json"));
  });
});
