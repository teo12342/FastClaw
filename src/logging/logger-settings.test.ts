...
let originalFastClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.FASTCLAW_TEST_FILE_LOG;
  originalFastClawLogLevel = process.env.FASTCLAW_LOG_LEVEL;
  delete process.env.FASTCLAW_TEST_FILE_LOG;
  delete process.env.FASTCLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.FASTCLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalFastClawLogLevel === undefined) {
    delete process.env.FASTCLAW_LOG_LEVEL;
  } else {
    process.env.FASTCLAW_LOG_LEVEL = originalFastClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.FASTCLAW_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();
