...
type ConfiguredBinding = NonNullable<FastClawConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<FastClawConfig>,
): FastClawConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: FastClawConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/fastclaw" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
