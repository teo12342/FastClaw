...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".fastclaw-profile-decorated");
}
 ...
 */
export function decorateFastClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_FASTCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_FASTCLAW_BROWSER_COLOR).toUpperCase();
