import type { BrowserConfig, BrowserProfileConfig, FastClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_FASTCLAW_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "fastclaw" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_FASTCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "fastclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_FASTCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_FASTCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an FastClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for fastclaw/fastclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: FastClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_FASTCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "fastclaw";
