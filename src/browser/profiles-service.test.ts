...
vi.mock("./chrome.js", () => ({
  resolveFastClawUserDataDir: vi.fn(() => "/tmp/fastclaw-test/fastclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveFastClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "fastclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveFastClawUserDataDir).mockReturnValue(userDataDir);
