...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const fastclaw = resolveProfile(resolved, "fastclaw");
    expect(fastclaw?.driver).toBe("fastclaw");
    expect(fastclaw?.cdpPort).toBe(18800);
    expect(fastclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from FASTCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ FASTCLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const fastclaw = resolveProfile(resolved, "fastclaw");
      expect(fastclaw?.cdpPort).toBe(19012);
      expect(fastclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ FASTCLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const fastclaw = resolveProfile(resolved, "fastclaw");
      expect(fastclaw?.cdpPort).toBe(19022);
      expect(fastclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "fastclaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        fastclaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("fastclaw");
  });
