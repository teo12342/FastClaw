  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-fastclaw-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-fastclaw-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-fastclaw-labels", "1");
          ...
          tag.setAttribute("data-fastclaw-labels", "1");
