...
function localFastClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "fastclaw",
...
function createLocalFastClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localFastClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveFastClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-reset-"));
    const profileDir = path.join(tempRoot, "fastclaw");
    ...
    const ops = createLocalFastClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveFastClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "fastclaw");
    fs.mkdirSync(profileDir, { recursive: true });
