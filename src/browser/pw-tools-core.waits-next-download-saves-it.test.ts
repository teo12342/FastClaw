...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredFastClawTmpDir: vi.fn(() => "/tmp/fastclaw"),
}));
vi.mock("../infra/tmp-fastclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredFastClawTmpDir.mockReturnValue("/tmp/fastclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredFastClawTmpDir.mockReturnValue("/tmp/fastclaw-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "fastclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredFastClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "fastclaw-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "fastclaw-preferred", "downloads")}${path.sep}`),
    );
  });
