  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          fastclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.FASTCLAW_GATEWAY_PORT;
    process.env.FASTCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.FASTCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.FASTCLAW_GATEWAY_PASSWORD;
