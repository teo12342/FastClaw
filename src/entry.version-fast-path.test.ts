...
describe("entry root version fast path", () => {
  ...
  beforeEach(() => {
    ...
    process.argv = ["node", "fastclaw", "--version"];
    exitSpy = vi
      .spyOn(process, "exit")
  ...
  it("prints commit-tagged version output when commit metadata is available", async () => {
    ...
    await vi.waitFor(() => {
      expect(logSpy).toHaveBeenCalledWith("FastClaw 9.9.9-test (abc1234)");
  ...
  it("falls back to plain version output when commit metadata is unavailable", async () => {
    ...
    await vi.waitFor(() => {
      expect(logSpy).toHaveBeenCalledWith("FastClaw 9.9.9-test");
      expect(exitSpy).toHaveBeenCalledWith(0);
    });
