...
const workspaceHarness = createCommandWorkspaceHarness("fastclaw-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".fastclaw", "extensions", params.pluginId);
...
function buildCfg(): FastClawConfig {
  return {
    plugins: {
      enabled: true,
    },
