...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop fastclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@fastclaw_bot", { botUsername: "fastclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@fastclaw_bot", { botUsername: "fastclaw_bot" })).toBe(true);
