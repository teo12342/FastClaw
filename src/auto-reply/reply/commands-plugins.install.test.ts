...
const workspaceHarness = createCommandWorkspaceHarness("fastclaw-command-plugins-install-");
...
describe("handleCommands /plugins install", () => {
  ...
  it("installs a plugin from a local path", async () => {
    ...
    await withTempHome("fastclaw-command-plugins-home-", async () => {
  ...
  it("installs from an explicit clawhub: spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.2.3",
      extensions: ["index.js"],
      packageName: "@fastclaw/clawhub-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@fastclaw/clawhub-demo",
    ...
    await withTempHome("fastclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildCommandTestParams(
        "/plugins install clawhub:@fastclaw/clawhub-demo@1.2.3",
        {
          commands: {
            text: true,
            plugins: true,
      ...
      expect(installPluginFromClawHubMock).toHaveBeenCalledWith(
        expect.objectContaining({
          spec: "clawhub:@fastclaw/clawhub-demo@1.2.3",
        }),
      );
      expect(persistPluginInstallMock).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it("treats /plugin add as an install alias", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.0.0",
      extensions: ["index.js"],
      packageName: "@fastclaw/alias-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@fastclaw/alias-demo",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.0.0",
    ...
    await withTempHome("fastclaw-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildCommandTestParams(
        "/plugin add clawhub:@fastclaw/alias-demo@1.0.0",
        {
          commands: {
            text: true,
            plugins: true,
      ...
      expect(installPluginFromClawHubMock).toHaveBeenCalledWith(
        expect.objectContaining({
          spec: "clawhub:@fastclaw/alias-demo@1.0.0",
        }),
      );
    });
  });
