...
const { createFastClawCodingToolsMock } = vi.hoisted(() => ({
  createFastClawCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    createFastClawCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createFastClawCodingToolsMock).toHaveBeenCalledWith(
