...
export function isCommandEnabled(cfg: FastClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: FastClawConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: FastClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: FastClawConfig) {
