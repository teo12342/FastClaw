...
function createReplyConfig(home: string, streamMode?: "block"): FastClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("fastclaw-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("FASTCLAW_TEST_FAST", "1");
