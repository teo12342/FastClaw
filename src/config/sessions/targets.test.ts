...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): FastClawConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: FastClawConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        FASTCLAW_STATE_DIR: envStateDir,
      };
      const cfg: FastClawConfig = {};
