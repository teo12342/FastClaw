...
function collectConfigEnvVarsByTarget(cfg?: FastClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: FastClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: FastClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: FastClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.FASTCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
