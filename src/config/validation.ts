...
import type { FastClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: FastClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: FastClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = FastClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as FastClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as FastClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as FastClawConfig,
  };
}
