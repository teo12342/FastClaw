...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("fastclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "fastclaw-sandbox-browser-global" },
      agentBrowser: { network: "fastclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("fastclaw-sandbox-browser-agent");
  });
