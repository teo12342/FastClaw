...
import type { FastClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareFastClawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  fastclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  fastclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: FastClawConfig,
  pathSegments: string[],
): { changed: boolean; next: FastClawConfig } {
...
function coerceConfig(value: unknown): FastClawConfig {
...
function stampConfigVersion(cfg: FastClawConfig): FastClawConfig {
...
function warnIfConfigFromFuture(cfg: FastClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareFastClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer FastClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as FastClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): FastClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as FastClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: FastClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as FastClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.FASTCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.FASTCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.FASTCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.FASTCLAW_WATCH_SESSION === "string" &&
        deps.env.FASTCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.FASTCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.FASTCLAW_WATCH_COMMAND === "string" &&
        deps.env.FASTCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.FASTCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `FASTCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: FastClawConfig | null = null;
let runtimeConfigSourceSnapshot: FastClawConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FASTCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FASTCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): FastClawConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): FastClawConfig {
