...
function isWhatsAppConfigured(cfg: FastClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: FastClawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: FastClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: FastClawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: FastClawConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: FastClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: FastClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: FastClawConfig, pluginId: string): FastClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
