...
import type { ConfigFileSnapshot } from "./types.fastclaw.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.fastclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/fastclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/fastclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/fastclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/fastclaw/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(FastClawSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = FastClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "FastClawConfig";
    const hints = mapSensitivePaths(FastClawSchema, "", {});
