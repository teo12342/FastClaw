...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.FASTCLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveFastClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    FASTCLAW_STATE_DIR: path.join(os.tmpdir(), "fastclaw-config-doc-baseline-state"),
    FASTCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
  ...
  const channelPlugins =
    process.env.FASTCLAW_CONFIG_DOC_BASELINE_DEBUG === "1"
