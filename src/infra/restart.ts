...
export function triggerFastClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.FASTCLAW_SYSTEMD_UNIT,
        process.env.FASTCLAW_PROFILE,
      );
  ...
  const label =
    process.env.FASTCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.FASTCLAW_PROFILE);
