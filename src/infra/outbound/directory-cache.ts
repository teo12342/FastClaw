...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: FastClawConfig | null = null;
  ...
  get(key: string, cfg: FastClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: FastClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: FastClawConfig): void {
