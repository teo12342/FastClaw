...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred FastClaw tmp root", async () => {
    const tmpRoot = resolvePreferredFastClawTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideFastClaw = path.join(os.tmpdir(), "outside-fastclaw", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
