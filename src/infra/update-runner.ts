...
const DEFAULT_PACKAGE_NAME = "fastclaw";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "fastclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("fastclaw doctor", doctorArgv, gitRoot, { FASTCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
