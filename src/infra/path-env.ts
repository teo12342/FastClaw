...
function candidateBinDirs(opts: EnsureFastClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `fastclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "fastclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.FASTCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "fastclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `fastclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureFastClawCliOnPath(opts: EnsureFastClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.FASTCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
