...
const GATEWAY_DEVICE_ID_HEADER = "x-fastclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-fastclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-fastclaw-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.FASTCLAW_APNS_RELAY_BASE_URL);
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.FASTCLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require FASTCLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.FASTCLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
