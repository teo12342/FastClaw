...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: FastClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: FastClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: FastClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: FastClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: FastClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: FastClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: FastClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
