...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.FASTCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.FASTCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set FASTCLAW_APNS_TEAM_ID and FASTCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.FASTCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.FASTCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.FASTCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set FASTCLAW_APNS_PRIVATE_KEY_P8 or FASTCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading FASTCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createFastClawPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    fastclaw: createFastClawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
