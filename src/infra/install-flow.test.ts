...
describe("resolveExistingInstallPath", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-install-flow-"));
  });
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "fastclaw-install-flow");
    ...
    expect(withTempDirSpy).toHaveBeenCalledWith("fastclaw-plugin-", expect.any(Function));
  ...
  it("returns extract failure when extraction throws", async () => {
    vi.spyOn(installSource, "withTempDir").mockImplementation(
      async (_prefix, fn) => await fn("/tmp/fastclaw-install-flow"),
    );
