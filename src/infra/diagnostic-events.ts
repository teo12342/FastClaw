...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __fastclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__fastclawDiagnosticEventsState) {
    globalStore.__fastclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__fastclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: FastClawConfig): boolean {
