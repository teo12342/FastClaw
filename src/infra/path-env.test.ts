...
let ensureFastClawCliOnPath: typeof import("./path-env.js").ensureFastClawCliOnPath;

describe("ensureFastClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "FASTCLAW_PATH_BOOTSTRAPPED",
    "FASTCLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureFastClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling fastclaw exists", () => {
    const tmp = abs("/tmp/fastclaw-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "fastclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.FASTCLAW_PATH_BOOTSTRAPPED;

    ensureFastClawCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.FASTCLAW_PATH_BOOTSTRAPPED = "1";
    ensureFastClawCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/fastclaw-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "fastclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/fastclaw-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "fastclaw");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "fastclaw");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/fastclaw-path/case-linuxbrew");
