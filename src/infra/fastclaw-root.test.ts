...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__fastclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "fastclaw-root");
...
describe("resolveFastClawPackageRoot", () => {
  let resolveFastClawPackageRoot: typeof import("./fastclaw-root.js").resolveFastClawPackageRoot;
  let resolveFastClawPackageRootSync: typeof import("./fastclaw-root.js").resolveFastClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveFastClawPackageRoot, resolveFastClawPackageRootSync } =
      await import("./fastclaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "fastclaw");
    const pkgRoot = path.join(project, "node_modules", "fastclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "fastclaw" }));

    expect(resolveFastClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "fastclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "fastclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "fastclaw" }));

    expect(resolveFastClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveFastClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-fastclaw package roots", async () => {
    const pkgRoot = fx("not-fastclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-fastclaw" }));

    expect(resolveFastClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveFastClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveFastClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
