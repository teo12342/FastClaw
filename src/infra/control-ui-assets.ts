...
import { resolveFastClawPackageRoot, resolveFastClawPackageRootSync } from "./fastclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveFastClawPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "fastclaw" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveFastClawPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // fastclaw.mjs or dist/<bundle>.js
