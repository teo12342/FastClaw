...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-run-node-"));
  try {
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        const exitCode = await runNodeMain({
          ...
          env: {
            ...process.env,
            FASTCLAW_FORCE_BUILD: "1",
            FASTCLAW_RUNNER_LOG: "0",
          },
          spawn,
        ...
        expect(nodeCalls).toEqual([[process.execPath, "fastclaw.mjs", "--version"]]);
      });
    },
  );
});
