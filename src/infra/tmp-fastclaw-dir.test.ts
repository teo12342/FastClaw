...
import { POSIX_FASTCLAW_TMP_DIR, resolvePreferredFastClawTmpDir } from "./tmp-fastclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredFastClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `fastclaw-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_FASTCLAW_TMP_DIR) {
  ...
  const resolved = resolvePreferredFastClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredFastClawTmpDir", () => {
  it("prefers /tmp/fastclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_FASTCLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/fastclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_FASTCLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/fastclaw when /tmp/fastclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/fastclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/fastclaw is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/fastclaw is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/fastclaw is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback FastClaw temp dir/);
  });
