...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/fastclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/fastclaw-state", "devices"),
      pendingPath: path.join("/tmp/fastclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/fastclaw-state", "devices", "paired.json"),
    });
  });
