...
const originalFastClawHome = process.env.FASTCLAW_HOME;
...
afterEach(() => {
  vi.restoreAllMocks();
  if (originalFastClawHome === undefined) {
    delete process.env.FASTCLAW_HOME;
  } else {
    process.env.FASTCLAW_HOME = originalFastClawHome;
  }
...
function createHomeDir(): string {
  ...
  tempDirs.push(dir);
  process.env.FASTCLAW_HOME = dir;
  return dir;
}

function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".fastclaw", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".fastclaw", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".fastclaw", "exec-approvals.sock")),
    );
  });
