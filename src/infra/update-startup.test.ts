...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["FASTCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.FASTCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveFastClawPackageRoot } = await import("./fastclaw-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveFastClawPackageRoot).mockResolvedValue("/opt/fastclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/fastclaw/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
