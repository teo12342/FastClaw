...
vi.mock("./fastclaw-root.js", () => ({
  resolveFastClawPackageRoot: vi.fn(async () => null),
  resolveFastClawPackageRootSync: vi.fn(() => null),
}));
...
let fastclawRoot: typeof import("./fastclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    fastclawRoot = await import("./fastclaw-root.js");
  });
  ...
  it("uses resolveFastClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/fastclaw");
    (
      fastclawRoot.resolveFastClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/fastclaw"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "fastclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "fastclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-fastclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/fastclaw-bundle");
    (
      fastclawRoot.resolveFastClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
