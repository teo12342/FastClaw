...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("fastclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("fastclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("fastclaw-lid-a-", (first) => {
      withTempDirSync("fastclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.fastclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "fastclaw-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers FASTCLAW_HOME over HOME", () => {
    vi.stubEnv("FASTCLAW_HOME", "/srv/fastclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/fastclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $FASTCLAW_HOME prefix when FASTCLAW_HOME is set", () => {
    vi.stubEnv("FASTCLAW_HOME", "/srv/fastclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/fastclaw-home")}/.fastclaw/fastclaw.json`)).toBe(
      "$FASTCLAW_HOME/.fastclaw/fastclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $FASTCLAW_HOME replacement when FASTCLAW_HOME is set", () => {
    vi.stubEnv("FASTCLAW_HOME", "/srv/fastclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/fastclaw-home")}/.fastclaw/fastclaw.json`),
    ).toBe("config: $FASTCLAW_HOME/.fastclaw/fastclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/fastclaw")).toBe(path.resolve(os.homedir(), "fastclaw"));
  });
  ...
  it("prefers FASTCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("FASTCLAW_HOME", "/srv/fastclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/fastclaw")).toBe(path.resolve("/srv/fastclaw-home", "fastclaw"));
