...
function buildConfigForFastClawTarget(entry: SecretRegistryEntry, envId: string): FastClawConfig {
  const config = {} as FastClawConfig;
  const refTargetPath =
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every fastclaw.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "fastclaw.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `FASTCLAW_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForFastClawTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `FASTCLAW_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
