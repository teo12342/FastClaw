...
import { resolveStateDir, type FastClawConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: FastClawConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.FASTCLAW_AUTH_STORE_READONLY;
  process.env.FASTCLAW_AUTH_STORE_READONLY = "1";
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as FastClawConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.FASTCLAW_AUTH_STORE_READONLY;
    } else {
      process.env.FASTCLAW_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
