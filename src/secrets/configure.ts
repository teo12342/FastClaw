...
function getSecretProviders(config: FastClawConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: FastClawConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: FastClawConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: FastClawConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: FastClawConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: FastClawConfig; next: FastClawConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in fastclaw.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "fastclaw secrets configure",
