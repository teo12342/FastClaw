...
export type ConfigureCandidate = {
  ...
  pathSegments: string[];
  label: string;
  configFile: "fastclaw.json" | "auth-profiles.json";
...
export function buildConfigureCandidates(config: FastClawConfig): ConfigureCandidate[] {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `fastclaw:${candidate.path}`;
}
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredFastClawConfig?: FastClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredFastClawConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const fastclawCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "fastclaw.json" as const,
  ...
  return [...fastclawCandidates, ...authCandidates].toSorted((a, b) =>
