...
import { loadConfig, type FastClawConfig } from "../config/config.js";
import {
...
describe("secrets runtime snapshot", () => {
  ...
  it("resolves env refs for config and auth profiles", async () => {
    ...
    const snapshot = await prepareSecretsRuntimeSnapshot({
      ...
      agentDirs: ["/tmp/fastclaw-agent-main"],
      loadAuthStore: () => ({
        version: 1,
        profiles: {
          "openai:default": {
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-secrets-file-provider-"));
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    ...
    const store = ensureAuthProfileStore("/tmp/fastclaw-agent-main");
  ...
  it("does not write inherited auth stores during runtime secret activation", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-secrets-runtime-"));
    ...
    const prevStateDir = process.env.FASTCLAW_STATE_DIR;
    ...
    try {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.FASTCLAW_STATE_DIR = prevStateDir;
      }
      await fs.rm(root, { recursive: true, force: true });
    }
  });
