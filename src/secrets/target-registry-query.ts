...
const FASTCLAW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of FASTCLAW_COMPILED_SECRET_TARGETS) {
...
const FASTCLAW_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: FASTCLAW_COMPILED_SECRET_TARGETS,
    entriesById: FASTCLAW_TARGETS_BY_ID,
  });
