...
const TEMP_MEDIA_PREFIX = "fastclaw-media-";
...
function createGroqAudioConfig(): FastClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): FastClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): FastClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: FastClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredFastClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        FASTCLAW_AGENT_DIR: isolatedAgentDir,
