...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "FASTCLAW_HOME", "FASTCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("fastclaw-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("fastclaw-bundle-mcp-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.FASTCLAW_HOME;
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "FASTCLAW_HOME", "FASTCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("fastclaw-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("fastclaw-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.FASTCLAW_HOME;
      delete process.env.FASTCLAW_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".fastclaw", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".fastclaw", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "FASTCLAW_HOME", "FASTCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("fastclaw-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "fastclaw-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.FASTCLAW_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".fastclaw", "extensions", "inline-claude");
