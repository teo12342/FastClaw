...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: FastClawPluginDefinition;
  register?: FastClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as FastClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as FastClawPluginDefinition;
...
export function loadFastClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
        ...
        ? {
            alias: {
              ...
              ...(pluginSdkAccountIdAlias
                ? { "fastclaw/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: FastClawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as FastClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
