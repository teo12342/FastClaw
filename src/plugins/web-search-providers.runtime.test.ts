...
let loadFastClawPluginsMock: ReturnType<typeof vi.fn>;
...
describe("resolvePluginWebSearchProviders", () => {
  beforeEach(async () => {
    ...
    loadFastClawPluginsMock = vi
      .spyOn(loaderModule, "loadFastClawPlugins")
  ...
  it("loads bundled providers through the plugin loader in alphabetical order", () => {
    ...
    expect(loadFastClawPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("memoizes snapshot provider resolution for the same config and env", () => {
    ...
    const env = { FASTCLAW_HOME: "/tmp/fastclaw-home" } as NodeJS.ProcessEnv;
  ...
  it("invalidates the snapshot cache when config or env contents change in place", () => {
    ...
    const env = {
      FASTCLAW_HOME: "/tmp/fastclaw-home-a",
    ...
    env.FASTCLAW_HOME = "/tmp/fastclaw-home-b";
    resolvePluginWebSearchProviders({
      config,
      env,
      bundledAllowlistCompat: true,
    ...
    expect(loadFastClawPluginsMock).toHaveBeenCalledTimes(2);
  });
  ...
  it("skips web-search snapshot memoization when plugin cache opt-outs are set", () => {
    ...
    const env = {
      FASTCLAW_HOME: "/tmp/fastclaw-home",
      FASTCLAW_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
    } as NodeJS.ProcessEnv;
  ...
  it("skips web-search snapshot memoization when discovery cache ttl is zero", () => {
    ...
    const env = {
      FASTCLAW_HOME: "/tmp/fastclaw-home",
      FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS: "0",
    } as NodeJS.ProcessEnv;
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const env = {
      FASTCLAW_HOME: "/tmp/fastclaw-home",
      FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5",
      FASTCLAW_PLUGIN_MANIFEST_CACHE_MS: "20",
    } as NodeJS.ProcessEnv;
  ...
  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    ...
    const env = {
      FASTCLAW_HOME: "/tmp/fastclaw-home",
      FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    } as NodeJS.ProcessEnv;
    ...
    env.FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS = "5";
