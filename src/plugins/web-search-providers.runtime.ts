...
import { loadFastClawPlugins } from "./loader.js";
...
function shouldUseWebSearchProviderSnapshotCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FASTCLAW_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim()) {
    return false;
  }
  if (env.FASTCLAW_DISABLE_PLUGIN_MANIFEST_CACHE?.trim()) {
    return false;
  }
  const discoveryCacheMs = env.FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (discoveryCacheMs === "0") {
    return false;
  }
  const manifestCacheMs = env.FASTCLAW_PLUGIN_MANIFEST_CACHE_MS?.trim();
...
function resolveWebSearchProviderSnapshotCacheTtlMs(env: NodeJS.ProcessEnv): number {
  const discoveryCacheMs = resolveCacheMs(
    env.FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS,
    DEFAULT_DISCOVERY_CACHE_MS,
  );
  const manifestCacheMs = resolveCacheMs(
    env.FASTCLAW_PLUGIN_MANIFEST_CACHE_MS,
...
function buildWebSearchSnapshotCacheKey(params: {
  ...
  return JSON.stringify({
    ...
    env: {
      FASTCLAW_BUNDLED_PLUGINS_DIR: params.env.FASTCLAW_BUNDLED_PLUGINS_DIR ?? "",
      FASTCLAW_DISABLE_PLUGIN_DISCOVERY_CACHE:
        params.env.FASTCLAW_DISABLE_PLUGIN_DISCOVERY_CACHE ?? "",
      FASTCLAW_DISABLE_PLUGIN_MANIFEST_CACHE:
        params.env.FASTCLAW_DISABLE_PLUGIN_MANIFEST_CACHE ?? "",
      FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS: params.env.FASTCLAW_PLUGIN_DISCOVERY_CACHE_MS ?? "",
      FASTCLAW_PLUGIN_MANIFEST_CACHE_MS: params.env.FASTCLAW_PLUGIN_MANIFEST_CACHE_MS ?? "",
      FASTCLAW_HOME: params.env.FASTCLAW_HOME ?? "",
      FASTCLAW_STATE_DIR: params.env.FASTCLAW_STATE_DIR ?? "",
      FASTCLAW_CONFIG_PATH: params.env.FASTCLAW_CONFIG_PATH ?? "",
...
export function resolvePluginWebSearchProviders(params: {
  ...
  const registry = loadFastClawPlugins({
    config,
    workspaceDir: params.workspaceDir,
    env,
