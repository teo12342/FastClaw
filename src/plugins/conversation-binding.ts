...
const APPROVALS_PATH = "~/.fastclaw/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "fastclaw-app-server:thread:",
  "fastclaw-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("fastclaw.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("fastclaw.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to FastClaw instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
