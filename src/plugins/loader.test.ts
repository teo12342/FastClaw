...
import { __testing, loadFastClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `fastclaw-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.FASTCLAW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "fastclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.FASTCLAW_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadFastClawPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadFastClawPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.FASTCLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.FASTCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadFastClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadFastClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.FASTCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ FASTCLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
