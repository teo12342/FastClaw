...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `fastclaw-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "fastclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("rejects manifest paths that escape plugin root via symlink", () => {
    ...
    const outsideManifest = path.join(outsideDir, "fastclaw.plugin.json");
    const linkedManifest = path.join(rootDir, "fastclaw.plugin.json");
