...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "fastclaw-sdk-alias-"));
let tempDirIndex = 0;
...
function createPluginSdkAliasFixture(params?: {
  ...
  const packageJson: Record<string, unknown> = {
    name: params?.packageName ?? "fastclaw",
    type: "module",
  };
  if (trustedRootIndicatorMode === "bin+marker") {
    packageJson.bin = {
      fastclaw: "fastclaw.mjs",
    };
  }
  ...
  if (trustedRootIndicatorMode === "bin+marker") {
    fs.writeFileSync(path.join(root, "fastclaw.mjs"), "export {};\n", "utf-8");
  }
...
function createExtensionApiAliasFixture(params?: { srcBody?: string; distBody?: string }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "fastclaw", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "fastclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      buildFixture: () => createPluginSdkAliasFixture(),
      modulePath: () => "/tmp/tsx-cache/fastclaw-loader.js",
      argv1: (root: string) => path.join(root, "fastclaw.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("does not derive plugin-sdk subpaths from cwd fallback when package root is not an FastClaw root", () => {
    const fixture = createPluginSdkAliasFixture({
      packageName: "moltbot",
  ...
  it("builds plugin-sdk aliases from the module being loaded, not the loader location", () => {
    ...
    expect(fs.realpathSync(sourceAliases["fastclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(sourceAliases["fastclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
    ...
    expect(fs.realpathSync(distAliases["fastclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
    expect(fs.realpathSync(distAliases["fastclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
      fs.realpathSync(path.join(fixture.root, "dist", "plugin-sdk", "channel-runtime.js")),
    );
  });

  it("does not resolve plugin-sdk alias files from cwd fallback when package root is not an FastClaw root", () => {
    const fixture = createPluginSdkAliasFixture({
      srcFile: "channel-runtime.ts",
      distFile: "channel-runtime.js",
      packageName: "moltbot",
  ...
  it("loads source runtime shims through the non-native Jiti boundary", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "fastclaw/plugin-sdk/infra-runtime";
...
`,
    ...
    const withAlias = createJiti(jitiBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "fastclaw/plugin-sdk/infra-runtime": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
