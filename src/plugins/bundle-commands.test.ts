...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("fastclaw-bundle-commands-home-");
      const workspaceDir = await tempHarness.createTempDir("fastclaw-bundle-commands-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.FASTCLAW_HOME;
      delete process.env.FASTCLAW_STATE_DIR;

      const pluginRoot = path.join(homeDir, ".fastclaw", "extensions", "compound-bundle");
