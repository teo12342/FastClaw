...
describe("bundled plugin metadata", () => {
  ...
  it("prefers built generated paths when present and falls back to source paths", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-bundled-plugin-metadata-"));
  ...
  it("supports check mode for stale generated artifacts", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-bundled-plugin-generated-"));
    tempDirs.push(tempRoot);
    ...
    writeJson(path.join(tempRoot, "extensions", "alpha", "fastclaw.plugin.json"), {
      id: "alpha",
