...
import { resolveFastClawPackageRootSync } from "../infra/fastclaw-root.js";
...
function hasTrustedFastClawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasFastClawBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("fastclaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.fastclaw === "string");
  const hasFastClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "fastclaw.mjs"));
  return hasCliEntryExport || hasFastClawBin || hasFastClawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedFastClawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveFastClawPackageRootSync({ cwd });
  const fromExplicitHints =
    params.argv1 || params.moduleUrl
      ? resolveFastClawPackageRootSync({
          cwd,
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({ modulePath })) {
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`fastclaw/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(modulePath: string): Record<string, string> {
  ...
  return {
    ...(extensionApiAlias ? { "fastclaw/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "fastclaw/plugin-sdk": pluginSdkAlias } : {}),
