...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "fastclaw skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this FastClaw runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires FastClaw >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
  ...
  try {
    ...
    return {
      ...
      clawhub: {
        ...
        clawhubUrl:
          params.baseUrl?.trim() ||
          process.env.FASTCLAW_CLAWHUB_URL?.trim() ||
          "https://clawhub.ai",
