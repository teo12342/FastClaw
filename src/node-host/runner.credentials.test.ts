...
function createRemoteGatewayTokenRefConfig(tokenId: string): FastClawConfig {
  return {
    secrets: {
      providers: {
        default: { source: "env" },
...
describe("resolveNodeHostGatewayCredentials", () => {
  ...
  it("prefers FASTCLAW_GATEWAY_TOKEN over configured refs", async () => {
    const config = createRemoteGatewayTokenRefConfig("REMOTE_GATEWAY_TOKEN");

    await withEnvAsync(
      {
        FASTCLAW_GATEWAY_TOKEN: "token-from-env",
        FASTCLAW_GATEWAY_PASSWORD: undefined,
        REMOTE_GATEWAY_TOKEN: "token-from-ref",
      },
      async () => {
