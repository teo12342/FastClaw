...
declare const __FASTCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "fastclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["FASTCLAW_VERSION"],
      env["FASTCLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current FastClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __FASTCLAW_VERSION__ === "string" && __FASTCLAW_VERSION__) ||
  process.env.FASTCLAW_BUNDLED_VERSION ||
