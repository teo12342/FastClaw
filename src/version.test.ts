...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "fastclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "fastclaw", version: "2.3.4" });
  ...
  it("ignores non-fastclaw package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers FASTCLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        FASTCLAW_VERSION: "9.9.9",
        FASTCLAW_SERVICE_VERSION: "2.2.2",
  ...
  it("uses service and package fallbacks and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        FASTCLAW_VERSION: "   ",
        FASTCLAW_SERVICE_VERSION: "  2.0.0  ",
