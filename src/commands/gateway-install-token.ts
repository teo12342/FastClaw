...
import { readConfigFileSnapshot, writeConfigFile, type FastClawConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("fastclaw config set gateway.auth.mode token")} or ${formatCliCommand("fastclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
