...
export function hasExplicitMemorySearchConfig(cfg: FastClawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: FastClawConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: FastClawConfig, agentId: string) => { store: { path: string } } | null;
