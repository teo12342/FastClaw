...
export function normalizeCompatibilityConfigValues(cfg: FastClawConfig): {
  config: FastClawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: FastClawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as FastClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as FastClawConfig["browser"],
    };
    changes.push(
