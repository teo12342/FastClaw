...
type EnvSnapshot = {
  HOME?: string;
  FASTCLAW_HOME?: string;
  FASTCLAW_STATE_DIR?: string;
  FASTCLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    FASTCLAW_HOME: process.env.FASTCLAW_HOME,
    FASTCLAW_STATE_DIR: process.env.FASTCLAW_STATE_DIR,
    FASTCLAW_OAUTH_DIR: process.env.FASTCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: FastClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: FastClawConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.FASTCLAW_HOME = tempHome;
    process.env.FASTCLAW_STATE_DIR = path.join(tempHome, ".fastclaw");
    delete process.env.FASTCLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.FASTCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when FASTCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.FASTCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: FastClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints fastclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: FastClawConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/fastclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/fastclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /fastclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
