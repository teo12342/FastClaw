...
import type { FastClawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: FastClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("fastclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.fastclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("fastclaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.fastclaw.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("fastclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("fastclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("fastclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("fastclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('fastclaw config set session.dmScope "per-channel-peer"') +
