...
beforeAll(() => {
  envSnapshot = captureEnv(["FASTCLAW_PROFILE"]);
  process.env.FASTCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/fastclaw-root.js", () => ({
  resolveFastClawPackageRoot: vi.fn().mockResolvedValue("/tmp/fastclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/fastclaw/pnpm-lock.yaml",
      markerPath: "/tmp/fastclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.fastclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.fastclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("FastClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("fastclaw status --all") ||
          l.includes("fastclaw --profile isolated status --all") ||
          l.includes("fastclaw status --all") ||
          l.includes("fastclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.FASTCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
