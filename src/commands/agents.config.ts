...
function resolveAgentName(cfg: FastClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: FastClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: FastClawConfig): AgentSummary[] {
