...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: FastClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: FastClawConfig) => { changed: boolean; next: FastClawConfig };
  defaultModel: string;
  overrideConfig: FastClawConfig;
  alreadyDefaultConfig: FastClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: FastClawConfig) => config,
      applyDefaultConfig: (config: FastClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: FastClawConfig = { agents: { defaults: {} } };
