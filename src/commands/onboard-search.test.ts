...
function createPerplexityConfig(apiKey: string, enabled?: boolean): FastClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: FastClawConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): FastClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: FastClawConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: FastClawConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
