 ...
 * Runs as part of `fastclaw doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("fastclaw config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("fastclaw memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("fastclaw configure --section model")}`,
        `- To disable: ${formatCliCommand("fastclaw config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("fastclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
