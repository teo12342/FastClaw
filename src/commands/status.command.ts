...
export async function statusCommand(
  ...
  runtime.log(theme.heading("FastClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`fastclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("fastclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("fastclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("fastclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("fastclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.fastclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.fastclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("fastclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("fastclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("fastclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("fastclaw gateway probe")}`);
  }
}
