...
function resolveMode(cfg: FastClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("FastClaw doctor");

  const root = await resolveFastClawPackageRoot({
  ...
  let cfg: FastClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("fastclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("fastclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("fastclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("fastclaw doctor --fix")}" to apply changes.`);
  }
