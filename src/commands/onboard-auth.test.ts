...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under FASTCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("fastclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-oauth-sync-"));
    ...
    process.env.FASTCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-oauth-nosync-"));
    process.env.FASTCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside FASTCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-oauth-external-"));
    process.env.FASTCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* FASTCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FASTCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("fastclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
