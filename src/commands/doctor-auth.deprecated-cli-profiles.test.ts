...
beforeEach(() => {
  envSnapshot = captureEnv(["FASTCLAW_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-auth-"));
...
describe("maybeRemoveDeprecatedCliAuthProfiles", () => {
  it("removes deprecated CLI auth profiles from store + config", async () => {
    ...
    const next = await maybeRemoveDeprecatedCliAuthProfiles(
      cfg as unknown as FastClawConfig,
      makePrompter(true),
    );
