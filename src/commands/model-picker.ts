...
type PromptDefaultModelResult = { model?: string; config?: FastClawConfig };
...
function resolveConfiguredModelRaw(cfg: FastClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: FastClawConfig): string[] {
...
export function applyPrimaryModel(cfg: FastClawConfig, model: string): FastClawConfig {
...
export function applyModelAllowlist(cfg: FastClawConfig, models: string[]): FastClawConfig {
  const defaults = cfg.agents?.defaults;
