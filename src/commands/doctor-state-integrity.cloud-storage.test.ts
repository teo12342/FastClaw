...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "FastClaw", ".fastclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/fastclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".fastclaw");
  ...
  it("anchors cloud detection to OS homedir when FASTCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".fastclaw");
    const originalFastClawHome = process.env.FASTCLAW_HOME;
    process.env.FASTCLAW_HOME = "/tmp/fastclaw-home-override";
