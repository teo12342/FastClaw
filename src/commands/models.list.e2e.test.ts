...
const ensureFastClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveFastClawAgentDir = vi.fn().mockReturnValue("/tmp/fastclaw-agent");
...
beforeEach(() => {
  ...
  ensureFastClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureFastClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureFastClawModelsJson).toHaveBeenCalled();
    expect(ensureFastClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
