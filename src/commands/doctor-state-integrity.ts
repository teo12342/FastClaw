...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".fastclaw"].map((dir) => path.resolve(root, entry.name, dir));
...
function shouldRequireOAuthDir(cfg: FastClawConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.FASTCLAW_OAUTH_DIR?.trim()) {
    return true;
  }
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".fastclaw");
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`fastclaw sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`fastclaw sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`fastclaw sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.fastclaw out of git; it contains credentials and session history.",
