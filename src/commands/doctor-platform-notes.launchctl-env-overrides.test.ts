...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "FASTCLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("FASTCLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv FASTCLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("FASTCLAW_GATEWAY_PASSWORD");
  });
