...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "fastclaw-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("fastclaw plugins install @fastclaw/matrix");
    expect(warnings[2]).toContain("fastclaw plugins install ./extensions/matrix");
  });
  ...
  it("removes stale Matrix plugin config when install path is missing", async () => {
    const missingPath = path.join(tmpdir(), "fastclaw-matrix-stale-cleanup-test-" + Date.now());
    await fs.rm(missingPath, { recursive: true, force: true });
