...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveFastClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("fastclaw models auth setup-token")}\` or \`${formatCliCommand("fastclaw configure")}\`.`
          : `Run \`${formatCliCommand("fastclaw configure")}\` or set an API key env var.`;
