...
export async function loadValidConfigOrThrow(): Promise<FastClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: FastClawConfig) => FastClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: FastClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: FastClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("fastclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
