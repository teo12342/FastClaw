...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<FastClawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<FastClawConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: FastClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: FastClawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

/** Build an env-backed SecretRef for a search provider. */
function buildSearchEnvRef(config: FastClawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: FastClawConfig, result: FastClawConfig): FastClawConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as FastClawConfig["plugins"],
  };
}
...
export async function setupSearch(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with fastclaw configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "FastClaw will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.fastclaw.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — FastClaw will store a reference instead of the API key.",
