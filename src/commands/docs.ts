...
const SEARCH_TOOL = "https://docs.fastclaw.ai/mcp.SearchFastClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.fastclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('fastclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.fastclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('fastclaw docs "your query"')}`);
    }
    return;
  }
