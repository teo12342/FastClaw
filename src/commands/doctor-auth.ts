...
function pruneAuthProfiles(
  cfg: FastClawConfig,
  profileIds: Set<string>,
): { next: FastClawConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("fastclaw models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("fastclaw models auth setup-token")} or ${formatCliCommand(
      "fastclaw configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "fastclaw models auth login --provider openai-codex",
    )} or ${formatCliCommand("fastclaw configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("fastclaw configure")}\` or \`${formatCliCommand("fastclaw onboard")}\`.`;
}
