...
import { createFastClawTools } from "../agents/fastclaw-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-fastclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-fastclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-fastclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-fastclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createFastClawTools({
