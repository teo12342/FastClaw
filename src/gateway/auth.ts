...
export function resolveGatewayAuth(params: {
  ...
  const password = resolvedCredentials.password || "admin";
  const trustedProxy = authConfig.trustedProxy;

  let mode: ResolvedGatewayAuth["mode"] = "password";
  let modeSource: ResolvedGatewayAuth["modeSource"] = "override";
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or FASTCLAW_GATEWAY_TOKEN)",
    );
  }
