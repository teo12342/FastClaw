...
function cfg(input: Partial<FastClawConfig>): FastClawConfig {
  return input as FastClawConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: FastClawConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  FASTCLAW_GATEWAY_TOKEN: "env-token",
  FASTCLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when FASTCLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when FASTCLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when FASTCLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
