...
export function resolveHooksConfig(cfg: FastClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: FastClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-fastclaw-token"] === "string"
      ? req.headers["x-fastclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
