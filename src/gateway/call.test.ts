...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("fastclaw doctor --fix");
  });
...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["FASTCLAW_GATEWAY_TOKEN", "FASTCLAW_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.FASTCLAW_GATEWAY_TOKEN = "env-token";
    process.env.FASTCLAW_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "FASTCLAW_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["FASTCLAW_GATEWAY_PASSWORD", "FASTCLAW_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.FASTCLAW_GATEWAY_PASSWORD = envPassword;
    }
