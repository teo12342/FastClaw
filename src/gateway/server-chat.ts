...
export function createAgentEventHandler({
  ...
  const emitChatDelta = (
    ...
    const cleanedRaw = stripInlineDirectiveTagsForDisplay(text).text;
    const cleaned = cleanedRaw ? applySafetyFilter(cleanedRaw) : cleanedRaw;
    if (!cleaned) {
      return;
    }
  ...
  const emitChatFinal = (
    ...
    const bufferedTextRaw = stripInlineDirectiveTagsForDisplay(
      chatRunState.buffers.get(clientRunId) ?? "",
    ).text.trim();
    const bufferedText = bufferedTextRaw ? applySafetyFilter(bufferedTextRaw) : bufferedTextRaw;
