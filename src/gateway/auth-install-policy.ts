...
type GatewayInstallAuthMode = NonNullable<NonNullable<FastClawConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: FastClawConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.FASTCLAW_GATEWAY_PASSWORD?.trim() ||
