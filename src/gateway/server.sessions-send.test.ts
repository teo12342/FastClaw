...
const { createFastClawTools } = await import("../agents/fastclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createFastClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createFastClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["FASTCLAW_GATEWAY_PORT", "FASTCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.FASTCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.FASTCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("FASTCLAW_CONFIG_PATH missing in gateway test environment");
      }
