...
import { resolveFastClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureFastClawModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, FastClawConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.FASTCLAW_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.FASTCLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.FASTCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.FASTCLAW_LIVE_GATEWAY_PROVIDERS);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.FASTCLAW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.FASTCLAW_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<FastClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.FASTCLAW_CONFIG_PATH,
    token: process.env.FASTCLAW_GATEWAY_TOKEN,
    skipChannels: process.env.FASTCLAW_SKIP_CHANNELS,
    skipGmail: process.env.FASTCLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.FASTCLAW_SKIP_CRON,
    skipCanvas: process.env.FASTCLAW_SKIP_CANVAS_HOST,
    agentDir: process.env.FASTCLAW_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.FASTCLAW_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.FASTCLAW_SKIP_CHANNELS = "1";
  process.env.FASTCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.FASTCLAW_SKIP_CRON = "1";
  process.env.FASTCLAW_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.FASTCLAW_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveFastClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-live-state-"));
  process.env.FASTCLAW_STATE_DIR = tempStateDir;
  ...
  process.env.FASTCLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.fastclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveFastClawAgentDir();
  const sanitizedCfg: FastClawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-live-"));
  const tempConfigPath = path.join(tempDir, "fastclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.FASTCLAW_CONFIG_PATH = tempConfigPath;

  await ensureFastClawModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "FastClaw live tool probe (local, safe): " +
    ...
    process.env.FASTCLAW_CONFIG_PATH = previous.configPath;
    process.env.FASTCLAW_GATEWAY_TOKEN = previous.token;
    process.env.FASTCLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.FASTCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.FASTCLAW_SKIP_CRON = previous.skipCron;
    process.env.FASTCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.FASTCLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.FASTCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureFastClawModelsJson(cfg);
      ...
      const rawModels = process.env.FASTCLAW_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via FASTCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.fastclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
