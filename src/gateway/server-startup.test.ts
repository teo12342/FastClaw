...
const ensureFastClawModelsJsonMock = vi.fn<
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveFastClawAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureFastClawModelsJson: (config: unknown, agentDir: unknown) =>
    ensureFastClawModelsJsonMock(config, agentDir),
}));
...
describe("gateway startup primary model warmup", () => {
  beforeEach(() => {
    ensureFastClawModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureFastClawModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureFastClawModelsJsonMock).not.toHaveBeenCalled();
