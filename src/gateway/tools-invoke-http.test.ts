...
let lastCreateFastClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/fastclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateFastClawToolsContext?.agentTo,
          agentThreadId: lastCreateFastClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createFastClawTools: (ctx: Record<string, unknown>) => {
      lastCreateFastClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateFastClawToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-fastclaw-message-to": "channel:24514",
        "x-fastclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
