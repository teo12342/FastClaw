...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "FASTCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "FASTCLAW_SKIP_GMAIL_WATCHER",
  "FASTCLAW_SKIP_CANVAS_HOST",
  "FASTCLAW_BUNDLED_PLUGINS_DIR",
  "FASTCLAW_SKIP_CHANNELS",
  "FASTCLAW_SKIP_PROVIDERS",
  "FASTCLAW_SKIP_CRON",
  "FASTCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.FASTCLAW_STATE_DIR = path.join(tempHome, ".fastclaw");
  delete process.env.FASTCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.FASTCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.FASTCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.FASTCLAW_SKIP_CANVAS_HOST = "1";
  process.env.FASTCLAW_SKIP_CHANNELS = "1";
  process.env.FASTCLAW_SKIP_PROVIDERS = "1";
  process.env.FASTCLAW_SKIP_CRON = "1";
  process.env.FASTCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.FASTCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "fastclaw-test-no-bundled-extensions")
    : "fastclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "fastclaw-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".fastclaw-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__fastclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__fastclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["FASTCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.FASTCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.FASTCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.FASTCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
