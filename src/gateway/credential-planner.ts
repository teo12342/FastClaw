...
type GatewaySecretDefaults = NonNullable<FastClawConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${FASTCLAW_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.FASTCLAW_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.FASTCLAW_GATEWAY_PASSWORD);
}
