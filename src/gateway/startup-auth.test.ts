...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: FastClawConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: FastClawConfig) {
  ...
  async function expectNoTokenGeneration(cfg: FastClawConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          FASTCLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
