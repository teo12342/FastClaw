...
describe("canvas host state dir defaults", () => {
  it("uses FASTCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("fastclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("FastClaw Canvas");
      } finally {
        await handler.close();
      }
    });
