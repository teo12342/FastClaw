...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `fastclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      FASTCLAW_LAUNCHD_LABEL: `ai.fastclaw.launchd-int-${testId}`,
      FASTCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
