...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.FASTCLAW_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service FASTCLAW_GATEWAY_TOKEN does not match gateway.auth.token in fastclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `fastclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
