...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.FASTCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.FASTCLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.FASTCLAW_SERVICE_MARKER).toBe("fastclaw");
    expect(env.FASTCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.FASTCLAW_SERVICE_VERSION).toBe("string");
    expect(env.FASTCLAW_SYSTEMD_UNIT).toBe("fastclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.FASTCLAW_LAUNCHD_LABEL).toBe("ai.fastclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", FASTCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.FASTCLAW_SYSTEMD_UNIT).toBe("fastclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.FASTCLAW_LAUNCHD_LABEL).toBe("ai.fastclaw.work");
    }
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".fastclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", FASTCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".fastclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", FASTCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".fastclaw"));
  });

  it("uses FASTCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", FASTCLAW_STATE_DIR: "/var/lib/fastclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/fastclaw"));
  });

  it("expands ~ in FASTCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", FASTCLAW_STATE_DIR: "~/fastclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/fastclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { FASTCLAW_STATE_DIR: "C:\\State\\fastclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\fastclaw");
  });
});
