...
function resolveSpeechProviderPluginEntries(cfg?: FastClawConfig): SpeechProviderPlugin[] {
  const active = getActivePluginRegistry();
  const registry =
    (active?.speechProviders?.length ?? 0) > 0 || !cfg
      ? active
      : loadFastClawPlugins({ config: cfg });
  return registry?.speechProviders?.map((entry) => entry.provider) ?? [];
}

function buildProviderMaps(cfg?: FastClawConfig): {
...
export function listSpeechProviders(cfg?: FastClawConfig): SpeechProviderPlugin[] {
