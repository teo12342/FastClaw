...
function prependInternalEventContext(
  ...
  if (body.includes("FastClaw runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("fastclaw agents list")}" to see configured agents.`,
      );
    }
  }
