...
describe("createFastClawCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-read-"));
    try {
      ...
      const contents = "Hello from fastclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createFastClawCodingTools({ sandbox });
