...
describe("sanitizeToolCallInputs redacts sessions_spawn attachments", () => {
  it("replaces attachments[].content with __FASTCLAW_REDACTED__", () => {
    ...
    expect(tool?.arguments?.attachments?.[0]?.content).toBe("__FASTCLAW_REDACTED__");
