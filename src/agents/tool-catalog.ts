...
type CoreToolDefinition = {
  ...
  includeInFastClawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const fastclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInFastClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:fastclaw": fastclawTools,
