...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    fastclaw: process.env.FASTCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.fastclaw === undefined) {
      delete process.env.FASTCLAW_GATEWAY_TOKEN;
    } else {
      process.env.FASTCLAW_GATEWAY_TOKEN = envSnapshot.fastclaw;
    }
  ...
  it("uses FASTCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.FASTCLAW_GATEWAY_TOKEN = "local-env-token";
