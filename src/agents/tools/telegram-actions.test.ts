...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): FastClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as FastClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): FastClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as FastClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: FastClawConfig, accountId = "media") {
    await handleTelegramAction(
