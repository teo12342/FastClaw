    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "fastclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "fastclaw.plugin.json"),
      },
    ],
  };
}
...
describe("resolvePluginSkillDirs", () => {
  it("keeps acpx plugin skills when ACP is enabled", async () => {
    const workspaceDir = await tempDirs.make("fastclaw-");
    const acpxRoot = await tempDirs.make("fastclaw-acpx-plugin-");
    const helperRoot = await tempDirs.make("fastclaw-helper-plugin-");
