...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseFastClawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveFastClawMetadata(
  frontmatter: ParsedSkillFrontmatter,
): FastClawSkillMetadata | undefined {
  const metadataObj = resolveFastClawManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveFastClawManifestRequires(metadataObj);
  const install = resolveFastClawManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveFastClawManifestOs(metadataObj);
  return {
