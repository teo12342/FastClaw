...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("fastclaw-ws-", async (workspaceDir) => {
      await withTempDir("fastclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createFastClawCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("fastclaw-ws-", async (workspaceDir) => {
      await withTempDir("fastclaw-override-", async (overrideDir) => {
        const tools = createFastClawCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("fastclaw-ws-", async (workspaceDir) => {
      const cfg: FastClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createFastClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-fastclaw.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("fastclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("fastclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createFastClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
