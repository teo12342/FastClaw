...
describe("provider attribution", () => {
  it("resolves the canonical FastClaw product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      FASTCLAW_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in FastClaw runtime wrapper.",
      product: "FastClaw",
      version: "2026.3.22",
      headers: {
        "HTTP-Referer": "https://fastclaw.ai",
        "X-OpenRouter-Title": "FastClaw",
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { FASTCLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "fastclaw",
        "User-Agent": "fastclaw/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { FASTCLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "fastclaw",
      "User-Agent": "fastclaw/2026.3.22",
    });
  });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { FASTCLAW_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ FASTCLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
