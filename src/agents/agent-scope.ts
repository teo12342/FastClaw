...
type AgentEntry = NonNullable<NonNullable<FastClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: FastClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: FastClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: FastClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: FastClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: FastClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: FastClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: FastClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
