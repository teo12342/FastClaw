...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createFastClawTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-test-"));

    try {
      await withEnvAsync(
        { FASTCLAW_STATE_DIR: stateDir, FASTCLAW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: fastclaw --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/fastclaw" } }\n}\n';
