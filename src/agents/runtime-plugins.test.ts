...
vi.mock("../plugins/loader.js", () => ({
  loadFastClawPlugins: hoisted.loadFastClawPlugins,
}));
...
describe("ensureRuntimePluginsLoaded", () => {
  beforeEach(() => {
    hoisted.loadFastClawPlugins.mockReset();
  ...
  it("does not reactivate plugins when a process already has an active registry", async () => {
    ...
    expect(hoisted.loadFastClawPlugins).not.toHaveBeenCalled();
  });
  ...
  it("loads runtime plugins when no active registry exists", async () => {
    ...
    expect(hoisted.loadFastClawPlugins).toHaveBeenCalledWith({
      config: {} as never,
      workspaceDir: "/tmp/workspace",
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
