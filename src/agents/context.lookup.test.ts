...
function mockContextDeps(params: {
  ...
  const ensureFastClawModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveFastClawAgentDir: () => "/tmp/fastclaw-agent",
  }));
  ...
  return { ensureFastClawModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real fastclaw startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "fastclaw", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "fastclaw", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "fastclaw", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureFastClawModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureFastClawModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
