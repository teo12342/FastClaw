...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async (importOriginal) => {
    ...
    return {
      ...mod,
      ensureFastClawModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): FastClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): FastClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): FastClawConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `fastclaw-auth-rotation-${Date.now()}.log`),
    });
