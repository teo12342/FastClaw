 ...
 *
 * @see https://github.com/fastclaw/fastclaw/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /fastclaw\.json|fastclaw gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify fastclaw.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
