...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["FASTCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-subagent-"));
  ...
  it("uses isolated temp state when FASTCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "fastclaw-test-state"));
  });
});
