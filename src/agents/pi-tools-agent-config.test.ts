...
import { createFastClawCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: FastClawConfig) {
    return createFastClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<FastClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<FastClawConfig["agents"]>["list"]>[number]["tools"];
  }): FastClawConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createFastClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createFastClawCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createFastClawCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createFastClawCodingTools({
      config: cfg,
    ...
    const bobTools = createFastClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createFastClawCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createFastClawCodingTools({
      config: cfg,
