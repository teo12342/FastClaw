...
const GOOGLE_LIVE_SINGLE_KEY = "FASTCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "FASTCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "FASTCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "FASTCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `FASTCLAW_LIVE_${base}_KEY`;
