...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.FASTCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".fastclaw", `workspace-${profile}`);
  }
  return path.join(home, ".fastclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".fastclaw";
