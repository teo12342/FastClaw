...
const FAST_TEST_MODE = process.env.FASTCLAW_TEST_FAST === "1";
    ...
    lines.push(
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to FastClaw sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`fastclaw ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for FastClaw subagents (`runtime: "subagent"`).',
