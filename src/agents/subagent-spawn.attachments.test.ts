...
let previousConfigPath = process.env.FASTCLAW_CONFIG_PATH;
...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(async () => {
    ...
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `fastclaw-subagent-attachments-${process.pid}-${Date.now()}-`),
    );
    configPathOverride = path.join(workspaceDirOverride, "fastclaw.test.json");
    fs.writeFileSync(configPathOverride, JSON.stringify(configOverride, null, 2));
    previousConfigPath = process.env.FASTCLAW_CONFIG_PATH;
    process.env.FASTCLAW_CONFIG_PATH = configPathOverride;
  });
  ...
  afterEach(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.FASTCLAW_CONFIG_PATH = previousConfigPath;
    }
    configPathOverride = "";
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".fastclaw", "attachments");
