...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "fastclaw-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.FASTCLAW_BUNDLED_SKILLS_DIR = bundledDir;
