...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("fastclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("fastclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("fastclaw-sbx-");
