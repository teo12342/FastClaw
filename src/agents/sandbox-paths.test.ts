...
import { resolvePreferredFastClawTmpDir } from "../infra/tmp-fastclaw-dir.js";
...
async function withOutsideHardlinkInFastClawTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside fastclaw tmp root",
      media: path.join(os.tmpdir(), "outside-fastclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked FastClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked FastClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInFastClawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked FastClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInFastClawTmp(
