...
let createFastClawTools: typeof import("./fastclaw-tools.js").createFastClawTools;

function findTool(name: string, config: FastClawConfig) {
  const allTools = createFastClawTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: FastClawConfig; env?: NodeJS.ProcessEnv }) {
...
describe("fastclaw tools runtime web metadata wiring", () => {
  const priorFetch = global.fetch;

  beforeEach(async () => {
    vi.resetModules();
    secretsRuntime = await import("../secrets/runtime.js");
    ({ createFastClawTools } = await import("./fastclaw-tools.js"));
  });
