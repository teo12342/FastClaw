...
let ensureFastClawModelsJson: typeof import("./models-config.js").ensureFastClawModelsJson;
...
beforeEach(async () => {
  ...
  ({ ensureFastClawModelsJson, resetModelsJsonReadyCacheForTest } =
...
function createOpenAiApiKeySourceConfig(): FastClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): FastClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): FastClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): FastClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function withGatewayTokenMode(config: FastClawConfig): FastClawConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function withGeneratedModelsFromRuntimeSource(
  params: {
    ...
    runtimeConfig: FastClawConfig;
    candidateConfig?: FastClawConfig;
  },
  ...
  await withTempHome(async () => {
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      try {
        ...
        await ensureFastClawModelsJson(params.candidateConfig ?? loadConfig());
        await runAssertions();
      } finally {
...
describe("models-config runtime source snapshot", () => {
  ...
  it("uses non-env marker from runtime source snapshot for file refs", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        unsetEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS);
        const sourceConfig: FastClawConfig = {
          models: {
            providers: {
              moonshot: {
        ...
        const runtimeConfig: FastClawConfig = {
          models: {
            providers: {
              moonshot: {
                baseUrl: "https://api.moonshot.ai/v1",
        ...
        try {
          ...
          await ensureFastClawModelsJson(loadConfig());
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const clonedRuntimeConfig: FastClawConfig = {
          ...runtimeConfig,
          agents: {
            defaults: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureFastClawModelsJson(clonedRuntimeConfig);
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const sourceConfig = createOpenAiApiKeySourceConfig();
        const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
        const firstCandidate: FastClawConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        const secondCandidate: FastClawConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureFastClawModelsJson(firstCandidate);
          ...
          await ensureFastClawModelsJson(secondCandidate);
  ...
  it("keeps source markers when runtime projection is skipped for incompatible top-level shape", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const incompatibleCandidate: FastClawConfig = {
          ...createOpenAiApiKeyRuntimeConfig(),
        };

        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureFastClawModelsJson(incompatibleCandidate);
          await expectGeneratedProviderApiKey("openai", "OPENAI_API_KEY"); // pragma: allowlist secret
        } finally {
          clearRuntimeConfigSnapshot();
          clearConfigCache();
