 ...
 * A common mistake — caused by the similarity with the `fastclaw.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (fastclaw.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.FASTCLAW_AUTH_STORE_READONLY === "1";
