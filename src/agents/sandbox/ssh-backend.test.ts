...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "fastclaw-test-ssh-config"),
    host: "fastclaw-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "fastclaw-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "fastclaw-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/fastclaw/fastclaw-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/fastclaw/fastclaw-ssh-agent-worker");
