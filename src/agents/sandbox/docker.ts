...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "fastclaw.sandbox=1");
  args.push("--label", `fastclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `fastclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `fastclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "fastclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`fastclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`fastclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("fastclaw sandbox recreate --all");
}
