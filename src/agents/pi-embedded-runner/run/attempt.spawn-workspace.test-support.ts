...
vi.mock("../../docs-path.js", () => ({
  resolveFastClawDocsPath: async () => undefined,
}));
...
vi.mock("../../pi-tools.js", () => ({
  createFastClawCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) => [
    {
      name: "sessions_spawn",
      execute: async (
        _callId: string,
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("fastclaw.cache-ttl", data),
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-ctx-engine-agent-"));
