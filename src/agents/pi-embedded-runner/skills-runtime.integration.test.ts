...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("fastclaw-bundled-");
  const workspaceDir = await createTempDir("fastclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: FastClawConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.FASTCLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
