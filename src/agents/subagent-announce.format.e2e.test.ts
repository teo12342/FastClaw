...
let configOverride: FastClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: FastClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/fastclaw/fastclaw/issues/31298
    previousFastTestEnv = process.env.FASTCLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.FASTCLAW_TEST_FAST;
      return;
    }
    process.env.FASTCLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // FASTCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("FastClaw runtime context (internal):");
