...
function resolveAttachmentLimits(config: FastClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".fastclaw", "attachments");
  const relDir = path.posix.join(".fastclaw", "attachments", attachmentId);
