...
type ModelsConfig = NonNullable<FastClawConfig["models"]>;
...
export async function ensureFastClawModelsJson(
  config?: FastClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveFastClawAgentDir();
