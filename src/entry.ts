...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "fastclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "fastclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.FASTCLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.FASTCLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.FASTCLAW_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[fastclaw] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[fastclaw] ${parsed.error}`);
      process.exit(2);
    }
