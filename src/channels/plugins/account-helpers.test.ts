...
function cfg(accounts?: Record<string, unknown> | null): FastClawConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as FastClawConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as FastClawConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as FastClawConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as FastClawConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as FastClawConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as FastClawConfig)).toBe("default");
    });
  });
});
