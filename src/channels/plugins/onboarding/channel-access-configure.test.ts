...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: FastClawConfig, policy: ChannelAccessPolicy) => FastClawConfig;
  resolveAllowlist: (params: { cfg: FastClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: FastClawConfig; resolved: TResolved }) => FastClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: FastClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: FastClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: FastClawConfig, policy: ChannelAccessPolicy): FastClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: FastClawConfig, policy: ChannelAccessPolicy): FastClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: FastClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: FastClawConfig; resolved: string[] }) => {
      calls.push("apply");
