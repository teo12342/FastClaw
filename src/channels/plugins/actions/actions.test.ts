...
function telegramCfg(): FastClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as FastClawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: FastClawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as FastClawConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as FastClawConfig;
...
function createSignalAccountOverrideCfg(): FastClawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as FastClawConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as FastClawConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as FastClawConfig,
        accountId: undefined,
        params: {
