...
describe("createStaticReplyToModeResolver", () => {
  it("always returns the configured mode", () => {
    expect(createStaticReplyToModeResolver("off")({ cfg: {} as FastClawConfig })).toBe("off");
    expect(createStaticReplyToModeResolver("all")({ cfg: {} as FastClawConfig })).toBe("all");
  });
});
...
describe("createTopLevelChannelReplyToModeResolver", () => {
  it("reads the top-level channel config", () => {
    ...
    expect(
      resolver({
        cfg: { channels: { discord: { replyToMode: "first" } } } as FastClawConfig,
      }),
    ).toBe("first");
  });
  ...
  it("falls back to off", () => {
    const resolver = createTopLevelChannelReplyToModeResolver("discord");
    expect(resolver({ cfg: {} as FastClawConfig })).toBe("off");
  });
});
...
describe("createScopedAccountReplyToModeResolver", () => {
  ...
  it("passes chatType through", () => {
    ...
    expect(resolver({ cfg: {} as FastClawConfig, chatType: "group" })).toBe("first");
