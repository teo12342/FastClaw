...
function baseCfg(): FastClawConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
...
function buildDispatchContextPayload(params: { cfg: FastClawConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@fastclaw hello",
      bodyText: "@fastclaw hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
  ...
  it("honors group allowlist and ignores pairing-store senders in groups", () => {
    ...
    const denied = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@fastclaw hi",
      bodyText: "@fastclaw hi",
      allowFrom: ["*"],
    ...
    const allowed = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@fastclaw ok",
      bodyText: "@fastclaw ok",
      allowFrom: ["*"],
      groupAllowFrom: ["chat_id:101"],
      groupPolicy: "allowlist",
      dmPolicy: "pairing",
