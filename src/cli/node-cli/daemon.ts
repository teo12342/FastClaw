...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("fastclaw node install"), formatCliCommand("fastclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("fastclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    FASTCLAW_LOG_PREFIX: baseEnv.FASTCLAW_LOG_PREFIX ?? "node",
