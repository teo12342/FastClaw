    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fastclaw memory status", "Show index and provider status."],
          ["fastclaw memory status --deep", "Probe embedding provider readiness."],
          ["fastclaw memory index --force", "Force a full reindex."],
          ['fastclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'fastclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["fastclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.fastclaw.ai/cli/memory")}\n`,
    );
