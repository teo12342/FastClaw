...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@fastclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@fastclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@fastclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@fastclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @fastclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@fastclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@fastclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/fastclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@fastclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @fastclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@fastclaw/plugin-alpha@latest",
      true,
      "/tmp/fastclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@fastclaw/plugin-alpha@latest",
      installPath: "/tmp/fastclaw/extensions/alpha",
      version: "1.2.3",
