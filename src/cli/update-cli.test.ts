...
import type { FastClawConfig, ConfigFileSnapshot } from "../config/types.fastclaw.js";
...
vi.mock("../infra/fastclaw-root.js", () => ({
  resolveFastClawPackageRoot: vi.fn(),
}));
...
const { resolveFastClawPackageRoot } = await import("../infra/fastclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/fastclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as FastClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/fastclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveFastClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("fastclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveFastClawPackageRoot).mockClear();
    ...
    vi.mocked(resolveFastClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("fastclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/fastclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "fastclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("FastClaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as FastClawConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveFastClawPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("fastclaw-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ FASTCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.FASTCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("fastclaw-update-wizard");
    await withEnvAsync({ FASTCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
