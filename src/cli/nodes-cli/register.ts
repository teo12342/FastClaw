    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fastclaw nodes status", "List known nodes with live status."],
          ["fastclaw nodes pairing pending", "Show pending node pairing requests."],
          ['fastclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["fastclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.fastclaw.ai/cli/nodes")}\n`,
    );
