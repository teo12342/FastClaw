...
import type { ConfigFileSnapshot } from "../config/types.fastclaw.js";

const loadConfigMock = vi.fn<() => FastClawConfig>();
...
vi.mock("../commands/doctor/providers/matrix.js", () => ({
  cleanStaleMatrixPluginConfig: (cfg: FastClawConfig) => cleanStaleMatrixPluginConfigMock(cfg),
}));
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    raw: '{ "plugins": {} }',
    parsed: { plugins: {} },
    resolved: { plugins: {} } as FastClawConfig,
    valid: false,
    config: { plugins: {} } as FastClawConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const matrixNpmRequest = {
    rawSpec: "@fastclaw/matrix",
    normalizedSpec: "@fastclaw/matrix",
  };
  ...
  beforeEach(() => {
    ...
    cleanStaleMatrixPluginConfigMock.mockImplementation((cfg: FastClawConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("returns the config directly when loadConfig succeeds", async () => {
    const cfg = { plugins: { entries: { matrix: { enabled: true } } } } as FastClawConfig;
  ...
  it("does not run stale Matrix cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as FastClawConfig;
    loadConfigMock.mockReturnValue(cfg);
  ...
  it("allows explicit repo-checkout Matrix reinstall recovery", async () => {
    ...
    const snapshotCfg = { plugins: {} } as FastClawConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Matrix install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `fastclaw doctor --fix` before installing plugins.");
    expect(readConfigFileSnapshotMock).not.toHaveBeenCalled();
  });
  ...
  it("throws when loadConfig fails with INVALID_CONFIG and snapshot parsed is empty", async () => {
    ...
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        parsed: {},
        config: {} as FastClawConfig,
      }),
    );

    await expect(loadConfigForInstall(matrixNpmRequest)).rejects.toThrow(
      "Config file could not be parsed; run `fastclaw doctor` to repair it.",
    );
  });
