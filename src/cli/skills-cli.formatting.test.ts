...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-bundled-skills-test-"));
    process.env.FASTCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
