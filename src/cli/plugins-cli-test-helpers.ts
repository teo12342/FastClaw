...
export const loadConfig = vi.fn<() => FastClawConfig>(() => ({}) as FastClawConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: FastClawConfig) => Promise<void>>(
  async () => undefined,
);
export const resolveStateDir = vi.fn(() => "/tmp/fastclaw-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: FastClawConfig) => writeConfigFile(config),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as FastClawConfig);
  readConfigFileSnapshot.mockResolvedValue({
    path: "/tmp/fastclaw-config.json5",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: {},
  ...
  resolveStateDir.mockReturnValue("/tmp/fastclaw-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: FastClawConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: FastClawConfig) => cfg);
  buildPluginStatusReport.mockReturnValue({
    plugins: [],
    diagnostics: [],
  });
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: FastClawConfig }) => ({
    config,
    warnings: [],
  }));
  ...
  recordHookInstall.mockImplementation((cfg: FastClawConfig) => cfg);
}
