...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    FASTCLAW_STATE_DIR: "/tmp/fastclaw-daemon",
    FASTCLAW_CONFIG_PATH: "/tmp/fastclaw-daemon/fastclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.FASTCLAW_STATE_DIR ?? "/tmp/fastclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.FASTCLAW_CONFIG_PATH ?? `${stateDir}/fastclaw.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/fastclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.FASTCLAW_STATE_DIR = "/tmp/fastclaw-cli";
    process.env.FASTCLAW_CONFIG_PATH = "/tmp/fastclaw-cli/fastclaw.json";
