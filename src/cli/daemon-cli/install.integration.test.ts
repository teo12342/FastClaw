...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("fastclaw-daemon-install-int-");
    configPath = path.join(tempHome, "fastclaw.json");
    process.env.HOME = tempHome;
    process.env.FASTCLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.FASTCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
