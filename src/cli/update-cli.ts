...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update FastClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["fastclaw update", "Update a source checkout (git)"],
        ["fastclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["fastclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["fastclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["fastclaw update --dry-run", "Preview actions without changing anything"],
        ["fastclaw update --no-restart", "Update without restarting the service"],
        ["fastclaw update --json", "Output result as JSON"],
        ["fastclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["fastclaw update wizard", "Interactive update wizard"],
        ["fastclaw --update", "Shorthand for fastclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run fastclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.fastclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fastclaw update status", "Show channel + version status."],
          ["fastclaw update status --json", "JSON output."],
          ["fastclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.fastclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
