...
import { resolveFastClawPackageRoot } from "../../infra/fastclaw-root.js";
...
const FASTCLAW_REPO_URL = "https://github.com/fastclaw/fastclaw.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "fastclaw";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("fastclaw@")) {
    return trimmed.slice("fastclaw@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.FASTCLAW_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveFastClawPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", FASTCLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `FASTCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set FASTCLAW_GIT_DIR to an empty folder or an fastclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`FASTCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "fastclaw.mjs");
