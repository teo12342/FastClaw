...
const STEP_LABELS: Record<string, string> = {
  ...
  "fastclaw doctor entry": "Checking doctor entrypoint",
  "fastclaw doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g fastclaw@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g fastclaw@latest --omit=optional");
  }
