...
describe("plugins cli update", () => {
  ...
  it("maps an explicit unscoped npm dist-tag update to the tracked plugin id", async () => {
    ...
    await runPluginsCommand(["plugins", "update", "fastclaw-codex-app-server@beta"]);
  ...
  it("maps an explicit scoped npm dist-tag update to the tracked plugin id", async () => {
      ...
      plugins: {
        installs: {
          "voice-call": {
            ...
            resolvedName: "@fastclaw/voice-call",
          },
        },
      },
    } as FastClawConfig;
    ...
    await runPluginsCommand(["plugins", "update", "@fastclaw/voice-call@beta"]);
    ...
    expect(updateNpmInstalledPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        specOverrides: {
          "voice-call": "@fastclaw/voice-call@beta",
        },
      }),
    );
  });
  ...
  it("maps an explicit npm version update to the tracked plugin id", async () => {
    ...
    await runPluginsCommand(["plugins", "update", "fastclaw-codex-app-server@0.2.0-beta.4"]);
  ...
  it("keeps using the recorded npm tag when update is invoked by plugin id", async () => {
    ...
    await runPluginsCommand(["plugins", "update", "fastclaw-codex-app-server"]);
