...
beforeEach(() => {
  ...
  originalHideBanner = process.env.FASTCLAW_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.FASTCLAW_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.FASTCLAW_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("fastclaw");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "fastclaw", "status", "--debug"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "fastclaw", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "fastclaw", "onboard"],
    });
  ...
  it("loads plugin registry for agents command", async () => {
    await runCommand({
      parseArgv: ["agents"],
      processArgv: ["node", "fastclaw", "agents"],
    });
  ...
  it("skips config guard for doctor, completion, and secrets commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "fastclaw", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "fastclaw", "completion"],
    });
    await runCommand({
      parseArgv: ["secrets"],
      processArgv: ["node", "fastclaw", "secrets"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "fastclaw", "--version"],
    });
  ...
  it("hides banner when FASTCLAW_HIDE_BANNER is truthy", async () => {
    process.env.FASTCLAW_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "fastclaw", "status"],
    });
