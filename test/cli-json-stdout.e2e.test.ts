  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.FASTCLAW_HOME;
        delete env.FASTCLAW_STATE_DIR;
        delete env.FASTCLAW_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "fastclaw.mjs");
