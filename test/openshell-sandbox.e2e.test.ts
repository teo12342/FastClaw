...
const FASTCLAW_OPENSHELL_E2E = process.env.FASTCLAW_E2E_OPENSHELL === "1";
const FASTCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const FASTCLAW_OPENSHELL_COMMAND =
  process.env.FASTCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "fastclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && FASTCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: FASTCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(FASTCLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "fastclaw-openshell-e2e-"));
      ...
      const gatewayName = `fastclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `fastclaw-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        browser: {
          enabled: false,
          image: "fastclaw-browser",
          containerPrefix: "fastclaw-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: FASTCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("fastclaw-openshell-e2e");
