...
import type { FastClawConfig } from "../../../extensions/discord/src/runtime-api.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: FastClawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): FastClawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("fastclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/acp-runtime")>(
    "fastclaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("fastclaw/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/command-auth")>(
...
vi.mock("fastclaw/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/reply-runtime")>(
    "fastclaw/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("fastclaw/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/config-runtime")>(
...
vi.mock("fastclaw/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/runtime-env")>(
    "fastclaw/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("fastclaw/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("fastclaw/plugin-sdk/infra-runtime")>(
