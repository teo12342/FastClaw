...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `fastclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".fastclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          FASTCLAW_SKIP_PROVIDERS: "1",
          FASTCLAW_SKIP_GMAIL_WATCHER: "1",
          FASTCLAW_SKIP_CRON: "1",
          FASTCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          FASTCLAW_SKIP_CANVAS_HOST: "1",
          FASTCLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
