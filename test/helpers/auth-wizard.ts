...
export function requireFastClawAgentDir(): string {
  const agentDir = process.env.FASTCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("FASTCLAW_AGENT_DIR not set");
  }
  return agentDir;
}
