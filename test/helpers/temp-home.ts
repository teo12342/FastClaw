...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    fastclawHome: process.env.FASTCLAW_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("FASTCLAW_HOME", snapshot.fastclawHome);
  restoreKey("FASTCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked FASTCLAW_HOME.
  delete process.env.FASTCLAW_HOME;
  process.env.FASTCLAW_STATE_DIR = path.join(base, ".fastclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "fastclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".fastclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
