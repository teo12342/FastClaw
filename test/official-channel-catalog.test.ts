...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("fastclaw-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@fastclaw/whatsapp",
      version: "2026.3.23",
      description: "FastClaw WhatsApp channel plugin",
      fastclaw: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("fastclaw-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@fastclaw/whatsapp",
      fastclaw: {
        channel: {
