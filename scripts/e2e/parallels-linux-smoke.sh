...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/fastclaw-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/fastclaw-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/fastclaw-install.sh
  guest_exec /usr/bin/env FASTCLAW_NO_ONBOARD=1 bash /tmp/fastclaw-install.sh "${version_args[@]}" --no-onboard
  guest_exec fastclaw --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec fastclaw --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" fastclaw onboard \
    --non-interactive \
    --mode local \
...
verify_local_turn() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" fastclaw agent \
    --local \
    --agent main \
    --message ping \
    --json
