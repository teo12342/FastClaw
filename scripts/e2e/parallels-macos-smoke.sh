...
INSTALL_URL="https://fastclaw.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_FASTCLAW_BIN="/opt/homebrew/bin/fastclaw"
GUEST_FASTCLAW_ENTRY="/opt/homebrew/lib/node_modules/fastclaw/fastclaw.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/fastclaw-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/fastclaw-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://fastclaw.ai/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: fastclaw@2026.3.13-beta.1
...
spawn {*}$cmd
send -- "printf '__FASTCLAW_READY__\\n'\r"
expect "__FASTCLAW_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/fastclaw-prl.sh <<'__FASTCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__FASTCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/fastclaw-prl.sh; rc=\$?; rm -f /tmp/fastclaw-prl.sh; printf '__FASTCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__FASTCLAW_RC__:(-?[0-9]+)} {
...
resolve_latest_version() {
  ...
  npm view fastclaw version --userconfig "$(mktemp)"
}
...
export FASTCLAW_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/fastclaw-install.sh
bash /tmp/fastclaw-install.sh${version_arg_q}
$GUEST_FASTCLAW_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_FASTCLAW_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/fastclaw-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/fastclaw-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/fastclaw"; check_path "\$root/fastclaw/extensions"; if [ -d "\$root/fastclaw/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/fastclaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
verify_gateway() {
  guest_current_user_cli "$GUEST_FASTCLAW_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_FASTCLAW_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_FASTCLAW_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_FASTCLAW_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli \
    "$GUEST_FASTCLAW_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_FASTCLAW_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from fastclaw dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/fastclaw-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>FastClaw Control</title>' /tmp/fastclaw-dashboard-smoke.html >/dev/null; then
      if grep -F '<fastclaw-app></fastclaw-app>' /tmp/fastclaw-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>FastClaw Control</title>' /tmp/fastclaw-dashboard-smoke.html >/dev/null
grep -F '<fastclaw-app></fastclaw-app>' /tmp/fastclaw-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/fastclaw-discord-token <<'__FASTCLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__FASTCLAW_TOKEN__
cat >/tmp/fastclaw-discord-guilds.json <<'__FASTCLAW_GUILDS__'
$guilds_json
__FASTCLAW_GUILDS__
token="\$(tr -d '\n' </tmp/fastclaw-discord-token)"
guilds_json="\$(cat /tmp/fastclaw-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_FASTCLAW_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_FASTCLAW_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_FASTCLAW_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_FASTCLAW_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_FASTCLAW_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_FASTCLAW_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_FASTCLAW_ENTRY channels status --probe --json
rm -f /tmp/fastclaw-discord-token /tmp/fastclaw-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "fastclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "fastclaw-main-upgrade.tgz"
