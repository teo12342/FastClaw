...
IMAGE_NAME="fastclaw-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    FASTCLAW_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    FASTCLAW_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export FASTCLAW_ENTRY

	  home_dir=$(mktemp -d "/tmp/fastclaw-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.fastclaw/extensions/demo-plugin"

  cat > "$HOME/.fastclaw/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.fastclaw/extensions/demo-plugin/fastclaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FASTCLAW_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/fastclaw-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@fastclaw/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/fastclaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$FASTCLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$FASTCLAW_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/fastclaw-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@fastclaw/demo-plugin-dir",
  "version": "0.0.1",
  "fastclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/fastclaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FASTCLAW_ENTRY" plugins install "$dir_plugin"
	  node "$FASTCLAW_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/fastclaw-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@fastclaw/demo-plugin-file",
  "version": "0.0.1",
  "fastclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/fastclaw.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FASTCLAW_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$FASTCLAW_ENTRY" plugins list --json > /tmp/plugins4.json
