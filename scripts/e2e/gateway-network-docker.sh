...
IMAGE_NAME="fastclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="fastclaw-net-e2e-$$"
GW_NAME="fastclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "FASTCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "FASTCLAW_SKIP_CHANNELS=1" \
  -e "FASTCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "FASTCLAW_SKIP_CRON=1" \
  -e "FASTCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
