...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/fastclaw-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: fastclaw@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/fastclaw-main-$CURRENT_HEAD_SHORT.tgz"
...
start_server() {
  ...
  ) >/tmp/fastclaw-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
run_windows_script_via_log() {
  ...
  runner_name="fastclaw-update-$RANDOM-$RANDOM.ps1"
  log_name="fastclaw-update-$RANDOM-$RANDOM.log"
  done_name="fastclaw-update-$RANDOM-$RANDOM.done"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/fastclaw-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/fastclaw-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/fastclaw-main-update.tgz
version="\$(/opt/homebrew/bin/fastclaw --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/fastclaw models set openai/gpt-5.4
/opt/homebrew/bin/fastclaw gateway status --deep --require-rpc
/opt/homebrew/bin/fastclaw agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/fastclaw-main-update.sh
}
...
\$env:PATH = "\$env:LOCALAPPDATA\FastClaw\deps\portable-git\cmd;\$env:LOCALAPPDATA\FastClaw\deps\portable-git\mingw64\bin;\$env:LOCALAPPDATA\FastClaw\deps\portable-git\usr\bin;\$env:PATH"
\$tgz = Join-Path \$env:TEMP 'fastclaw-main-update.tgz'
...
& \$fastclaw gateway restart >> \$log 2>&1
Start-Sleep -Seconds 5
& \$fastclaw gateway status --deep --require-rpc >> \$log 2>&1
\$output = & \$fastclaw agent --agent main --session-id parallels-npm-update-windows-$head_short --message 'Reply with exact ASCII text OK only.' --json 2>&1
if (\$null -ne \$output) {
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/fastclaw-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/fastclaw-main-update.tgz
npm install -g /tmp/fastclaw-main-update.tgz --no-fund --no-audit
version="\$(fastclaw --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
...
fastclaw models set openai/gpt-5.4
fastclaw agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" /bin/bash /tmp/fastclaw-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="fastclaw@$LATEST_VERSION"
fi
