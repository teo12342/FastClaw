...
usage() {
  cat <<'EOF'
Usage:
  FASTCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com \
...
- regenerates apps/ios/FastClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${FASTCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid FASTCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid FASTCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid FASTCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid FASTCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing FASTCLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(FASTCLAW_URL_SLASH)$(FASTCLAW_URL_SLASH)#g'
)"
...
FASTCLAW_CODE_SIGN_STYLE = Automatic
FASTCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
FASTCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
FASTCLAW_APP_BUNDLE_ID = ai.fastclaw.client
FASTCLAW_SHARE_BUNDLE_ID = ai.fastclaw.client.share
FASTCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.fastclaw.client.activitywidget
FASTCLAW_WATCH_APP_BUNDLE_ID = ai.fastclaw.client.watchkitapp
FASTCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.fastclaw.client.watchkitapp.extension
FASTCLAW_APP_PROFILE =
FASTCLAW_SHARE_PROFILE =
FASTCLAW_PUSH_TRANSPORT = relay
FASTCLAW_PUSH_DISTRIBUTION = official
FASTCLAW_URL_SLASH = /
FASTCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
FASTCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
