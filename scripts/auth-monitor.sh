...
#
# Suggested cron: */30 * * * * /home/admin/fastclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FastClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., fastclaw-alerts)
...
STATE_FILE="$HOME/.fastclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FastClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use fastclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FastClaw to $NOTIFY_PHONE..."
            fastclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FastClaw is down. Run: ssh l36 '~/fastclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
