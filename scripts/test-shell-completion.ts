 ...
 * `fastclaw update`. Use it to verify the completion installation flow
...
\`fastclaw update\`, \`fastclaw doctor\`, and \`fastclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# FastClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`fastclaw completion --install\` later to enable.`));
    return;
  }
