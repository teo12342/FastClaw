...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./fastclaw-npm-release-check.ts";
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `fastclaw@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`fastclaw@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `fastclaw-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "fastclaw");
    ...
    console.log(`fastclaw-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
function main(): void {
  ...
  if (!version) {
    throw new Error(
      "Usage: node --import tsx scripts/fastclaw-npm-postpublish-verify.ts <version>",
    );
  }
  ...
  console.log(
    `fastclaw-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(
      `fastclaw-npm-postpublish-verify: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exitCode = 1;
  }
}
