...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/fastclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "FastClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/fastclaw/scripts/mobile-reauth.sh'" --id fastclaw-auth
        ;;
    *)
