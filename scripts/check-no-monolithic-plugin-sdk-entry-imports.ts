...
import { discoverFastClawPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']fastclaw\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']fastclaw\/plugin-sdk\/compat["']/;
...
function main() {
  ...
  if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic fastclaw/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy fastclaw/plugin-sdk/compat.",
      );
    ...
    if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
      console.error(
        "Use fastclaw/plugin-sdk/<domain> or fastclaw/plugin-sdk/<channel> subpaths for bundled plugins; root and compat are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
