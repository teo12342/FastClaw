#!/usr/bin/env bash
# ClawDock - Docker helpers for FastClaw
# Inspired by Simon Willison's "Running FastClaw in Docker"
# https://til.simonwillison.net/llms/fastclaw-docker
#
# Installation:
...
# Common paths to check for FastClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/fastclaw"
  "${HOME}/workspace/fastclaw"
  "${HOME}/projects/fastclaw"
  "${HOME}/dev/fastclaw"
  "${HOME}/code/fastclaw"
  "${HOME}/src/fastclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found FastClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/fastclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ FastClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/fastclaw/fastclaw.git ~/fastclaw"
    echo "  cd ~/fastclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^FASTCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d fastclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart fastclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f fastclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.fastclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec fastclaw-gateway \
    bash -c 'echo "alias fastclaw=\"./fastclaw.mjs\"" > /tmp/.bashrc_fastclaw && bash --rcfile /tmp/.bashrc_fastclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm fastclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build fastclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "FASTCLAW_GATEWAY_TOKEN=$token" fastclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" fastclaw-gateway \
    bash -c './fastclaw.mjs config set gateway.remote.token "$TOKEN" && ./fastclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec fastclaw-gateway \
    bash -c "./fastclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart fastclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm fastclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec fastclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'fastclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for FastClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (fastclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to fastclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.fastclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'fastclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'fastclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.fastclaw.ai${_CLR_RESET}"
  echo ""
}
