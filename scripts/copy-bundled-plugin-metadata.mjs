...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "fastclaw.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "fastclaw.plugin.json");
    ...
    if (packageJson.fastclaw && "extensions" in packageJson.fastclaw) {
      packageJson.fastclaw = {
        ...packageJson.fastclaw,
        extensions: rewritePackageExtensions(packageJson.fastclaw.extensions),
        ...(typeof packageJson.fastclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.fastclaw.setupEntry) }
          : {}),
      };
    }
