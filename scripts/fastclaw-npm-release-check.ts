...
const EXPECTED_REPOSITORY_URL = "https://github.com/fastclaw/fastclaw";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "fastclaw") {
    errors.push(`package.json name must be "fastclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.fastclaw !== "fastclaw.mjs") {
    errors.push(
      `package.json bin.fastclaw must be "fastclaw.mjs"; found "${pkg.bin?.fastclaw ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`fastclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `fastclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
