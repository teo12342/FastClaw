...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.fastclaw?.extensions)
    ? packageJson.fastclaw.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.fastclaw?.setupEntry === "string" &&
    packageJson.fastclaw.setupEntry.trim().length > 0
      ? packageJson.fastclaw.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasPackageJson, sourceEntries } of entries) {
    artifacts.add(`dist/extensions/${id}/fastclaw.plugin.json`);
    if (hasPackageJson) {
