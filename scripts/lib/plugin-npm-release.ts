...
import { parseReleaseVersion } from "../fastclaw-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const extensions = packageJson.fastclaw?.extensions ?? [];

  if (!packageName.startsWith("@fastclaw/")) {
    errors.push(
      `package name must start with "@fastclaw/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("fastclaw.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("fastclaw.extensions must contain only non-empty strings.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const dir of dirs) {
    ...
    if (packageJson.fastclaw?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: packageJson.fastclaw?.install?.npmSpec?.trim() || undefined,
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "fastclaw-plugin-npm-view-"));
