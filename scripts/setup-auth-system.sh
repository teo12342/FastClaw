#!/bin/bash
# Setup FastClaw Auth Management System
...
echo "=== FastClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'fastclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  FastClaw message: Send warning via FastClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/fastclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/fastclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/fastclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now fastclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status fastclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
