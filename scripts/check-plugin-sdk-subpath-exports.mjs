...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("fastclaw/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("fastclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced fastclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced fastclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
