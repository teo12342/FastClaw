...
const shouldBuild = (deps) => {
  if (deps.env.FASTCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.FASTCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[fastclaw] ${message}\n`);
};

const runFastClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["fastclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
