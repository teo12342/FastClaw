...
const warningFilterKey = Symbol.for("fastclaw.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadFastClawPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadFastClawPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fastclaw-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@fastclaw/build-smoke-plugin",
      type: "module",
      fastclaw: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'fastclaw/plugin-sdk';",
...
const registry = loadFastClawPlugins({
  ...
  env: {
    ...process.env,
    FASTCLAW_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
