...
# Build and bundle FastClaw into a minimal .app we can open.
# Outputs to dist/FastClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/FastClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="FastClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.fastclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/fastclaw/fastclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/FastClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :FastClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :FastClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/FastClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/FastClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/FastClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/FastClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/FastClaw/Resources/FastClaw.icns" "$APP_ROOT/Contents/Resources/FastClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/FastClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying FastClawKit resources"
FASTCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/FastClawKit_FastClawKit.bundle"
if [ -d "$FASTCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/FastClawKit_FastClawKit.bundle"
  cp -R "$FASTCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/FastClawKit_FastClawKit.bundle"
else
  echo "WARN: FastClawKit resource bundle not found at $FASTCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running FastClaw"
killall -q FastClaw 2>/dev/null || true
