...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.fastclaw) ? packageJson.fastclaw : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.fastclaw?.channel?.id) || left.name;
    const rightId = trimString(right.fastclaw?.channel?.id) || right.name;
