# FastClaw Installer for Windows (PowerShell)
# Usage: iwr -useb https://fastclaw.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://fastclaw.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\fastclaw",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 FastClaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one FastClaw.$NC" -Level info
    Write-Host ""
}
...
function Install-FastClawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing FastClaw from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/fastclaw/fastclaw.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\fastclaw\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\fastclaw.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install FastClaw from git to $GitDir" -Level info
        } else {
            Install-FastClawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install FastClaw via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-FastClawNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'fastclaw onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 FastClaw installed successfully!" -Level success
}
