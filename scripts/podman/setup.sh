...
if [[ -n "${FASTCLAW_UID:-}" && -d /run/user ]] && command -v systemctl &>/dev/null; then
  if [[ ! -d "/run/user/$FASTCLAW_UID" ]]; then
    run_root install -d -m 700 -o "$FASTCLAW_UID" -g "$FASTCLAW_UID" "/run/user/$FASTCLAW_UID" || true
  fi
  run_root mkdir -p "/run/user/$FASTCLAW_UID/containers" || true
  run_root chown "$FASTCLAW_UID:$FASTCLAW_UID" "/run/user/$FASTCLAW_UID/containers" || true
  run_root chmod 700 "/run/user/$FASTCLAW_UID/containers" || true
fi

mkdir_user_dirs_as_fastclaw() {
  run_root install -d -m 700 -o "$FASTCLAW_UID" -g "$FASTCLAW_UID" "$FASTCLAW_HOME" "$FASTCLAW_CONFIG"
  run_root install -d -m 700 -o "$FASTCLAW_UID" -g "$FASTCLAW_UID" "$FASTCLAW_CONFIG/workspace"
}
...
ensure_subid_entry() {
  ...
  grep -q "^${FASTCLAW_USER}:" "$file" 2>/dev/null
}

if ! ensure_subid_entry /etc/subuid || ! ensure_subid_entry /etc/subgid; then
  echo "WARNING: ${FASTCLAW_USER} may not have subuid/subgid ranges configured." >&2
  echo "If rootless Podman fails, add 'fastclaw:100000:65536' to both /etc/subuid and /etc/subgid." >&2
fi
...
IMAGE_TAR_DIR="$(mktemp -d "${IMAGE_TMP_BASE%/}/fastclaw-podman-image.XXXXXX")"
chmod 700 "$IMAGE_TAR_DIR"
IMAGE_TAR="$IMAGE_TAR_DIR/fastclaw-image.tar"
cleanup_image_tar() {
  rm -rf "$IMAGE_TAR_DIR"
}
...
BUILD_ARGS=()
if [[ -n "${FASTCLAW_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "FASTCLAW_DOCKER_APT_PACKAGES=${FASTCLAW_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${FASTCLAW_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "FASTCLAW_EXTENSIONS=${FASTCLAW_EXTENSIONS}")
fi

echo "Building image fastclaw:local..."
podman build -t fastclaw:local -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]}" "$REPO_PATH"
echo "Saving image to $IMAGE_TAR ..."
podman save -o "$IMAGE_TAR" fastclaw:local

echo "Loading image into $FASTCLAW_USER Podman store..."
run_as_fastclaw podman load -i "$IMAGE_TAR"

echo "Installing launch script to $LAUNCH_SCRIPT_DST ..."
run_root install -m 0755 -o "$FASTCLAW_UID" -g "$FASTCLAW_UID" "$RUN_SCRIPT_SRC" "$LAUNCH_SCRIPT_DST"

if [[ ! -f "$FASTCLAW_CONFIG/.env" ]]; then
  TOKEN="$(generate_token_hex_32)"
  run_as_fastclaw sh -lc "umask 077 && printf '%s\n' 'FASTCLAW_GATEWAY_TOKEN=$TOKEN' > '$FASTCLAW_CONFIG/.env'"
  echo "Generated FASTCLAW_GATEWAY_TOKEN and wrote it to $FASTCLAW_CONFIG/.env"
fi

if [[ ! -f "$FASTCLAW_CONFIG/fastclaw.json" ]]; then
  run_as_fastclaw sh -lc "umask 077 && cat > '$FASTCLAW_CONFIG/fastclaw.json' <<'JSON'
{ \"gateway\": { \"mode\": \"local\" } }
JSON"
  echo "Wrote minimal config to $FASTCLAW_CONFIG/fastclaw.json"
fi

if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$FASTCLAW_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/fastclaw.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  run_as_fastclaw mkdir -p "$QUADLET_DIR"
  FASTCLAW_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$FASTCLAW_HOME")"
  sed "s|{{FASTCLAW_HOME}}|$FASTCLAW_HOME_ESCAPED|g" "$QUADLET_TEMPLATE" | \
    run_as_fastclaw sh -lc "cat > '$QUADLET_DST'"
  run_as_fastclaw chmod 0644 "$QUADLET_DST"

  echo "Reloading and enabling user service..."
  run_root systemctl --machine "${FASTCLAW_USER}@" --user daemon-reload
  run_root systemctl --machine "${FASTCLAW_USER}@" --user enable --now fastclaw.service
...
echo
echo "Next:"
echo "  ./scripts/run-fastclaw-podman.sh launch"
echo "  ./scripts/run-fastclaw-podman.sh launch setup"
