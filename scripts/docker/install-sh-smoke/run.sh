...
INSTALL_URL="${FASTCLAW_INSTALL_URL:-https://fastclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${FASTCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${FASTCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="fastclaw"
PACKAGE_NAME="${FASTCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (FASTCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${FASTCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${FASTCLAW_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
