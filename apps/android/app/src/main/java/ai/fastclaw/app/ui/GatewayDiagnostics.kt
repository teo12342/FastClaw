...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this FastClaw Android gateway connection failure.

    Please:
    - pick one route only: same machine, same LAN, Tailscale, or public URL
    - classify this as pairing/auth, TLS trust, wrong advertised route, wrong address/port, or gateway down
    - quote the exact app status/error below
    - tell me whether `fastclaw devices list` should show a pending pairing request
    - if more signal is needed, ask for `fastclaw qr --json`, `fastclaw devices list`, and `fastclaw nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("FastClaw gateway diagnostics", report))
