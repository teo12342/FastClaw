package ai.fastclaw.android.voice
...
class VoiceWakeCommandExtractorTest {
  ...
  fun extractsCommandAfterTriggerWord() {
    val res = VoiceWakeCommandExtractor.extractCommand("Claude take a photo", listOf("fastclaw", "claude"))
    assertEquals("take a photo", res)
  }

  @Test
  fun extractsCommandWithPunctuation() {
    val res = VoiceWakeCommandExtractor.extractCommand("hey fastclaw, what's the weather?", listOf("fastclaw"))
