...
class GatewaySessionInvokeTimeoutTest {
  ...
  fun replaceCanvasCapabilityInScopedHostUrl_rewritesTerminalCapabilitySegment() {
    assertEquals(
      "http://127.0.0.1:18789/__fastclaw__/cap/new-token",
      replaceCanvasCapabilityInScopedHostUrl(
        "http://127.0.0.1:18789/__fastclaw__/cap/old-token",
        "new-token",
      ),
    )
  }
  ...
  fun replaceCanvasCapabilityInScopedHostUrl_rewritesWhenQueryAndFragmentPresent() {
    assertEquals(
      "http://127.0.0.1:18789/__fastclaw__/cap/new-token?a=1#frag",
      replaceCanvasCapabilityInScopedHostUrl(
        "http://127.0.0.1:18789/__fastclaw__/cap/old-token?a=1#frag",
        "new-token",
      ),
    )
  }
