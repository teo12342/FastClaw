...
struct IOSGatewayChatTransport: FastClawChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.fastclaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> FastClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(FastClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> FastClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(FastClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [FastClawChatAttachmentPayload]) async throws -> FastClawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [FastClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(FastClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(FastClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FastClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FastClawChatEventPayload.self)
                        {
