...
final class RemindersService: RemindersServicing {
    func list(params: FastClawRemindersListParams) async throws -> FastClawRemindersListPayload {
        ...
        let payload: [FastClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return FastClawReminderPayload(
        ...
        return FastClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: FastClawRemindersAddParams) async throws -> FastClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = FastClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return FastClawRemindersAddPayload(reminder: payload)
    }
