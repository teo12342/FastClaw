...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> FastClawDeviceStatusPayload {
        ...
        return FastClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> FastClawDeviceInfoPayload {
        ...
        return FastClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> FastClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: FastClawBatteryState = switch device.batteryState {
        ...
        return FastClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> FastClawThermalStatusPayload {
        let state: FastClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return FastClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> FastClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return FastClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
