...
final class ContactsService: ContactsServicing {
    ...
    func search(params: FastClawContactsSearchParams) async throws -> FastClawContactsSearchPayload {
        ...
        return FastClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: FastClawContactsAddParams) async throws -> FastClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return FastClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return FastClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> FastClawContactPayload {
        FastClawContactPayload(
