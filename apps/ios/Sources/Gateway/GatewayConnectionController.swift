...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [FastClawCapability.canvas.rawValue, FastClawCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(FastClawCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(FastClawCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = FastClawLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(FastClawCapability.location.rawValue) }

        caps.append(FastClawCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(FastClawCapability.watch.rawValue)
        }
        caps.append(FastClawCapability.photos.rawValue)
        caps.append(FastClawCapability.contacts.rawValue)
        caps.append(FastClawCapability.calendar.rawValue)
        caps.append(FastClawCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(FastClawCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            FastClawScreenCommand.record.rawValue,
            FastClawSystemCommand.notify.rawValue,
            FastClawChatCommand.push.rawValue,
        ...
        if caps.contains(FastClawCapability.camera.rawValue) {
            commands.append(FastClawCameraCommand.list.rawValue)
            commands.append(FastClawCameraCommand.snap.rawValue)
            commands.append(FastClawCameraCommand.clip.rawValue)
        }
        if caps.contains(FastClawCapability.location.rawValue) {
            commands.append(FastClawLocationCommand.get.rawValue)
        }
        if caps.contains(FastClawCapability.device.rawValue) {
            commands.append(FastClawDeviceCommand.status.rawValue)
            commands.append(FastClawDeviceCommand.info.rawValue)
        }
        if caps.contains(FastClawCapability.watch.rawValue) {
            commands.append(FastClawWatchCommand.status.rawValue)
            commands.append(FastClawWatchCommand.notify.rawValue)
        }
        if caps.contains(FastClawCapability.photos.rawValue) {
            commands.append(FastClawPhotosCommand.latest.rawValue)
        }
        if caps.contains(FastClawCapability.contacts.rawValue) {
            commands.append(FastClawContactsCommand.search.rawValue)
            commands.append(FastClawContactsCommand.add.rawValue)
        }
        if caps.contains(FastClawCapability.calendar.rawValue) {
            commands.append(FastClawCalendarCommand.events.rawValue)
            commands.append(FastClawCalendarCommand.add.rawValue)
        }
        if caps.contains(FastClawCapability.reminders.rawValue) {
            commands.append(FastClawRemindersCommand.list.rawValue)
            commands.append(FastClawRemindersCommand.add.rawValue)
        }
        if caps.contains(FastClawCapability.motion.rawValue) {
            commands.append(FastClawMotionCommand.activity.rawValue)
            commands.append(FastClawMotionCommand.pedometer.rawValue)
        }
