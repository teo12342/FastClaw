...
final class MotionService: MotionServicing {
    func activities(params: FastClawMotionActivityParams) async throws -> FastClawMotionActivityPayload {
        ...
        let mapped: [FastClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return FastClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: FastClawPedometerParams) async throws -> FastClawPedometerPayload {
        ...
        let payload: FastClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = FastClawPedometerPayload(
