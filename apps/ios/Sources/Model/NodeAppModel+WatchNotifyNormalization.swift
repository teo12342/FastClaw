...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: FastClawWatchNotifyParams) -> FastClawWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [FastClawWatchAction]?,
        kind: String?,
        promptId: String?) -> [FastClawWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> FastClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return FastClawWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                FastClawWatchAction(id: "approve", label: "Approve"),
                FastClawWatchAction(id: "decline", label: "Decline", style: "destructive"),
                FastClawWatchAction(id: "open_phone", label: "Open iPhone"),
                FastClawWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            FastClawWatchAction(id: "done", label: "Done"),
            FastClawWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            FastClawWatchAction(id: "open_phone", label: "Open iPhone"),
            FastClawWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: FastClawWatchRisk?,
        priority: FastClawNotificationPriority?) -> FastClawWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: FastClawNotificationPriority?,
        risk: FastClawWatchRisk?) -> FastClawNotificationPriority?
    {
