...
struct PushBuildConfig {
    ...
    init(bundle: Bundle = .main) {
        self.transport = Self.readEnum(
            bundle: bundle,
            key: "FastClawPushTransport",
            fallback: .direct)
        self.distribution = Self.readEnum(
            bundle: bundle,
            key: "FastClawPushDistribution",
            fallback: .local)
        self.apnsEnvironment = Self.readEnum(
            bundle: bundle,
            key: "FastClawPushAPNsEnvironment",
            fallback: Self.defaultAPNsEnvironment)
        self.relayBaseURL = Self.readURL(bundle: bundle, key: "FastClawPushRelayBaseURL")
    }
