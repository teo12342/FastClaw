...
@MainActor
final class FastClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.fastclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.fastclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.fastclaw.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "fastclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "fastclaw.watch.promptId"
    static let sessionKeyKey = "fastclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "fastclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "fastclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "fastclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "fastclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "fastclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "fastclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "fastclaw.watch.action."
    static let actionIDKeyPrefix = "fastclaw.watch.action.id."
    static let actionLabelKeyPrefix = "fastclaw.watch.action.label."
    static let categoryPrefix = "fastclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: FastClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> FastClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return FastClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "FastClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [FastClawWatchAction]) -> [UNNotificationAction] {
...
@main
struct FastClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(FastClawAppDelegate.self) private var appDelegate
...
extension FastClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("FastClaw: installing uncaught exception handler")
