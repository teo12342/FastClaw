...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__FASTCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<FastClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.fastclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = FastClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> FastClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return FastClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> FastClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> FastClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return FastClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> FastClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = FastClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: FastClawTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: FastClawAgentEventPayload.self
            ) else {
                continue
            }
