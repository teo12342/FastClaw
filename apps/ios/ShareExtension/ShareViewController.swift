...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.fastclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to FastClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to FastClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to FastClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "FastClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "FastClaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "FastClaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("fastclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
