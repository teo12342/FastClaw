...
private let chatUILogger = Logger(subsystem: "ai.fastclaw", category: "FastClawChatUI")

@MainActor
@Observable
public final class FastClawChatViewModel {
    public private(set) var messages: [FastClawChatMessage] = []
    ...
    public var attachments: [FastClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [FastClawChatPendingToolCall] = []
    public private(set) var sessions: [FastClawChatSessionEntry] = []
    private let transport: any FastClawChatTransport
    ...
    private var pendingToolCallsById: [String: FastClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any FastClawChatTransport) {
    ...
    public var sessionChoices: [FastClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [FastClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: FastClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [FastClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: FastClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: FastClawChatMessage) -> FastClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> FastClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return FastClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: FastClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [FastClawChatMessage],
        incoming: [FastClawChatMessage]) -> [FastClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [FastClawChatMessage]) -> [FastClawChatMessage] {
        var result: [FastClawChatMessage] = []
    ...
    private static func dedupeKey(for message: FastClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [FastClawChatMessageContent] = [
            FastClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> FastClawChatAttachmentPayload in
            FastClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> FastClawChatSessionEntry {
        FastClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: FastClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: FastClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: FastClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = FastClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> FastClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
