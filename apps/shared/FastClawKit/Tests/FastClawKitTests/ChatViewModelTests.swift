...
import Foundation
import Testing
@testable import FastClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, FastClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [FastClawChatHistoryPayload]
    private let sessionsResponses: [FastClawChatSessionsListResponse]

    private let stream: AsyncStream<FastClawChatTransportEvent>
    private let continuation: AsyncStream<FastClawChatTransportEvent>.Continuation

    init(
        historyResponses: [FastClawChatHistoryPayload],
        sessionsResponses: [FastClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<FastClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> FastClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? FastClawChatHistoryPayload(
        ...
        attachments _: [FastClawChatAttachmentPayload]) async throws -> FastClawChatSendResponse
    {
        ...
        return FastClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> FastClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? FastClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: FastClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = FastClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = FastClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { FastClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = FastClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = FastClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = FastClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { FastClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
