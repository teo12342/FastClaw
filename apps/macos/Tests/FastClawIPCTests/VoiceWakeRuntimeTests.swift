...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["fastclaw"]
        let text = "hey fastclaw"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey fastclaw do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["fastclaw"], minPostTriggerGap: 0.3)
