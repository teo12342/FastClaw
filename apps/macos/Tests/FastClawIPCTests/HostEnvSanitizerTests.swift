...
struct HostEnvSanitizerTests {
    ...
    @Test func sanitizeShellWrapperAllowsOnlyExplicitOverrideKeys() {
        ...
        #expect(env["LANG"] == "C")
        #expect(env["LC_ALL"] == "C")
        #expect(env["FASTCLAW_TOKEN"] == nil)
        #expect(env["PS4"] == nil)
    }

    @Test func sanitizeNonShellWrapperKeepsRegularOverrides() {
        let env = HostEnvSanitizer.sanitize(overrides: ["FASTCLAW_TOKEN": "secret"])
        #expect(env["FASTCLAW_TOKEN"] == "secret")
    }
}
