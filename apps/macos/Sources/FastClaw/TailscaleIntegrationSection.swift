...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.fastclaw/fastclaw.json. Prefer FASTCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.fastclaw/fastclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.fastclaw/fastclaw.json. Restart the gateway to apply."
        }
