...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.fastclaw.mac`) even if the shipped
    ...
    {
        if environment["FASTCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "fastclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "fastclaw.nixMode") { return true }
