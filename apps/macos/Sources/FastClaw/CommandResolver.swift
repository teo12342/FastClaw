...
enum CommandResolver {
    private static let projectRootDefaultsKey = "fastclaw.gatewayProjectRootPath"
    private static let helperName = "fastclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let fastclawEntry = root.appendingPathComponent("fastclaw.mjs").path
        if FileManager().isReadableFile(atPath: fastclawEntry) { return fastclawEntry }
        let binEntry = root.appendingPathComponent("bin/fastclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/fastclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let fastclawPaths = self.fastclawManagedPaths(home: home)
        if !fastclawPaths.isEmpty {
            extras.insert(contentsOf: fastclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + fastclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func fastclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".fastclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func fastclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectFastClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("fastclaw.mjs").path,
            root.appendingPathComponent("bin/fastclaw.js").path,
        ]
    ...
    static func hasAnyFastClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.fastclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func fastclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let fastclawPath = self.projectFastClawExecutable(projectRoot: root) {
            return [fastclawPath, subcommand] + extraArgs
        }
        if let fastclawPath = self.fastclawExecutable(searchPaths: searchPaths) {
            return [fastclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "fastclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            fastclaw entrypoint missing (looked for dist/index.js or fastclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real fastclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/fastclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v fastclaw >/dev/null 2>&1; then
          CLI="$(command -v fastclaw)"
          fastclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/fastclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/fastclaw.mjs"
            node "$PRJ/fastclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/fastclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/fastclaw.js"
            node "$PRJ/bin/fastclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent fastclaw"
          pnpm --silent fastclaw \(quotedArgs);
        else
          echo "fastclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? FastClawConfigFile.loadDict()
