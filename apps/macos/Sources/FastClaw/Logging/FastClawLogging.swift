...
enum FastClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = FastClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = FastClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        FastClawLogging.bootstrapIfNeeded()
        let label = FastClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct FastClawOSLogHandler: LogHandler {
...
struct FastClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = FastClawLogging.parseLabel(self.label)
