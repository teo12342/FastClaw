...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.fastclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: FastClawPeekabooBridgeServices?
    private static var fastclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("FastClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = FastClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.fastclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.fastclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class FastClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.fastclaw.peekaboo")
