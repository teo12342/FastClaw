...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["FASTCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/fastclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("fastclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("fastclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/fastclaw/ (rolling or stdout), or nil if none exist.
