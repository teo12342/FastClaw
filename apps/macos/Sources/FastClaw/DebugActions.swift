...
enum DebugActions {
    private static let verboseDefaultsKey = "fastclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "fastclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = FastClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "FastClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FastClawPaths.configURL
        guard
