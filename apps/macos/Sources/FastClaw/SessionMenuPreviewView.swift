...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.fastclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> FastClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: FastClawSessionPreviewEntry,
    ...
    private static func cache(payload: FastClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: FastClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [FastClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [FastClawKit.AnyCodable]) -> [FastClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(FastClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: FastClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: FastClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: FastClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: FastClawChatMessage) -> String? {
