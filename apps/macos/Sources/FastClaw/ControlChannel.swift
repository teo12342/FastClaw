...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: FastClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.fastclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: FastClawKit.AnyCodable]()) {
                $0[$1.key] = FastClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: FastClawProtocol.AnyCodable?) -> [String: FastClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: FastClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: FastClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: FastClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("fastclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("fastclaw.control.agent")
}
