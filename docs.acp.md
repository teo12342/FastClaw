...
This document describes how the FastClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`fastclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
FastClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `fastclaw acp` and speaks ACP messages over stdio.
...
`fastclaw acp` resolves the Gateway URL and auth from CLI flags or config:
