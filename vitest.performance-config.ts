...
export function loadVitestExperimentalConfig(env: EnvMap = process.env): {
  ...
  if (!isDisabled(env.FASTCLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (isEnabled(env.FASTCLAW_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.FASTCLAW_VITEST_PRINT_IMPORT_BREAKDOWN)) {
